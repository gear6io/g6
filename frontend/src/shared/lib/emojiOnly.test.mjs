...
test("detects known custom emoji-only shortcode messages", () => {
  assert.equal(isEmojiOnlyMessage(":g6:", CUSTOM_EMOJI), true);
  assert.equal(isEmojiOnlyMessage(":g6: :party_parrot:", CUSTOM_EMOJI), true);
  assert.equal(isEmojiOnlyMessage(":G6:", CUSTOM_EMOJI), true);
});

test("allows mixed unicode and custom emoji", () => {
  assert.equal(isEmojiOnlyMessage("😀 :g6: ❤️", CUSTOM_EMOJI), true);
});
