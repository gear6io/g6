...
const CHANNELS = ["g6-bugs", "g6 dev", "general"];
...
test("triggers at start of string", () => {
  assert.deepEqual(at("#", "#g6", CHANNELS), {
    query: "g6",
    startIndex: 0,
  });
});
...
test("triggers after whitespace", () => {
  assert.deepEqual(at("#", "go to #g6", CHANNELS), {
    query: "g6",
    startIndex: 6,
  });
});
...
test("does NOT trigger when glued to a word character", () => {
  assert.equal(at("#", "foo#g6", CHANNELS), null);
...
test("multi-word channel name after ( resolves via multi-word path", () => {
  assert.deepEqual(at("#", "(#g6 de", CHANNELS), {
    query: "g6 de",
    startIndex: 1,
  });
});
...
test("multi-word: glued-to-word prefix still rejected", () => {
  // `x#g6 de` — `#` preceded by `x`, no boundary → no query even though
  // "g6 de" is a prefix of a known channel.
  assert.equal(at("#", "x#g6 de", CHANNELS), null);
});
