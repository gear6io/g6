...
export function imageReserveStyle(args: {
  ...
  if (hiddenSpoilerMediaSize) {
    ...
    return {
      "--g6-spoiler-media-aspect-ratio": ratio,
      "--g6-spoiler-media-width": `${hiddenSpoilerMediaSize.width}px`,
      aspectRatio: ratio,
      height: "auto",
...
export function isInsideHiddenSpoiler(element: Element): boolean {
  return (
    element.closest('.g6-spoiler[data-spoiler][data-revealed="false"]') !==
    null
  );
}

/**
 * `urlTransform` for `<ReactMarkdown>` that preserves `g6://message?…`
