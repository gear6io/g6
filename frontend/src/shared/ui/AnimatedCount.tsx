...
function AnimatedCountSlot({
  ...
  if (direction === 0 || previous === current) {
    return (
      <span
        className={cn(
          "g6-animated-count__slot",
          isDigit && "g6-animated-count__slot--digit",
        )}
      >
  ...
  if (!isDigit) {
    return (
      <span
        className="g6-animated-count__symbol"
  ...
  return (
    <span
      className="g6-animated-count__slot g6-animated-count__slot--digit"
      data-direction={direction > 0 ? "up" : "down"}
    >
      <span
        className="g6-animated-count__reel"
        key={`${version}-${previous}-${current}`}
      >
        {direction > 0 ? (
          <>
...
export function AnimatedCount({ className, value }: AnimatedCountProps) {
  ...
  return (
    <span className={cn("g6-animated-count", className)}>
      <span className="sr-only">{transition.current}</span>
      <span aria-hidden className="g6-animated-count__motion">
        {slots.map((slot) => (
          <AnimatedCountSlot
      ...
      <span aria-hidden className="g6-animated-count__static">
        {transition.current}
      </span>
    </span>
  );
