...
/**
 * Inline card rendered when the desktop detects a `g6:config-nudge`
  ...
  switch (requirement.surface) {
    ...
    case "git_bash":
      return (
        <div className="flex items-center gap-2 text-xs leading-4 text-muted-foreground">
          <span className="flex-1 [overflow-wrap:anywhere]">
            Git for Windows is required for g6-agent shell tools
          </span>
        </div>
      );
    case "cli_config_invalid": {
      // Config-invalid rows are purely informational — the user must edit an
      // external file. No Agent runtimes CTA (Gear6 can't repair ~/.codex/config.toml)
      // and no Edit Agent CTA (the field isn't managed by Gear6).
