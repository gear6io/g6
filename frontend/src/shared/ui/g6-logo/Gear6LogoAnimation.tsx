...
export type Gear6LogoAnimationProps = {
  ariaLabel?: string;
  className?: string;
  fullScreen?: boolean;
  loop?: boolean;
...
export default function Gear6LogoAnimation({
  ariaLabel = "Gear6 logo animation",
  className = "",
  fullScreen = true,
  loop = false,
  loopRestSeconds = 0,
  ...
  const maskId = `g6-logo-cutouts-${idSuffix}`;
  const textureId = `g6-logo-texture-${idSuffix}`;
  const texture = TEXTURES[config.texture ?? "soft"] ?? TEXTURES.soft;
  const classes = [
    "g6-logo",
    fullScreen && "g6-logo--screen",
    !fullScreen && "g6-logo--compact",
    showBackground && "g6-logo--background",
    className,
  ]
    .filter(Boolean)
    .join(" ");
