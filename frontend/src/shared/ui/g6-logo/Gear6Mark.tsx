...
/**
 * The finished Gear6 mark as a plain static SVG — no SMIL, no scripting, no
 * animation machinery. Geometry matches the final keyframe of the
 * Gear6LogoAnimation morph (v8 variant), rendered in `currentColor`, so it
...
export function Gear6Mark({ className }: { className?: string }) {
  const maskId = `g6-mark-cutouts-${useId().replace(/[^a-zA-Z0-9_-]/g, "")}`;

  return (
    <svg
      aria-hidden="true"
      className={["g6-mark", className].filter(Boolean).join(" ")}
      viewBox="0 0 466 309"
      fill="currentColor"
    >
      <defs>
