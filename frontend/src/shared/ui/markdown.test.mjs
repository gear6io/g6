...
// which would break copy → paste → click for `g6://message?…` links
// end-to-end. We pass a custom `urlTransform` that delegates to the
// default for `g6://message` and legacy `g6://message` hrefs.
//
...
test("messageLinkUrlTransform: preserves g6://message href", () => {
  const html = renderMarkdown(
    "Click [here](g6://message?channel=abc&id=xyz)",
  );
  // HTML-encoded `&` in attributes is fine — the browser decodes back to `&`.
  assert.match(html, /href="g6:\/\/message\?channel=abc&(?:amp;)?id=xyz"/);
});

test("messageLinkUrlTransform: preserves g6://message autolink href", () => {
  const html = renderMarkdown("<g6://message?channel=abc&id=xyz>");
  assert.match(html, /href="g6:\/\/message\?channel=abc&(?:amp;)?id=xyz"/);
});

test("messageLinkUrlTransform: preserves g6://message href with thread", () => {
  const html = renderMarkdown(
    "[link](g6://message?channel=c1&id=m1&thread=t1)",
  );
  assert.match(html, /href="g6:\/\/message\?[^"]*thread=t1"/);
});
...
test("messageLinkUrlTransform: preserves legacy g6://message href", () => {
  const html = renderMarkdown(
    "Click [here](g6://message?channel=abc&id=xyz)",
  );
  assert.match(html, /href="g6:\/\/message\?channel=abc&(?:amp;)?id=xyz"/);
...
test("messageLinkUrlTransform: leaves non-message g6:// schemes to default", () => {
  // `g6://connect?relay=…` is handled by a different code path (Tauri
  ...
  const html = renderMarkdown(
    "[connect](g6://connect?relay=wss://relay.example)",
  );
  assert.match(html, /href=""/);
});
...
// `remark-gfm`'s autolinker only covers http(s)://, so bare `g6://message`
...
test("remarkMessageLinks: bare g6://message URL is replaced", () => {
  const tree = runPlugin(paragraph(text("g6://message?channel=c&id=m")));
  const para = tree.children[0];
  assert.equal(para.children.length, 1);
  assert.equal(para.children[0].type, "message-link");
  assert.equal(para.children[0].value, "g6://message?channel=c&id=m");
  assert.equal(para.children[0].data.hName, "message-link");
});

test("remarkMessageLinks: legacy bare g6://message URL is replaced", () => {
  const tree = runPlugin(paragraph(text("g6://message?channel=c&id=m")));
  const para = tree.children[0];
  assert.equal(para.children.length, 1);
  assert.equal(para.children[0].type, "message-link");
...
test("remarkMessageLinks: mid-sentence URL splits surrounding text", () => {
  const tree = runPlugin(
    paragraph(text("see g6://message?channel=c&id=m here")),
  );
...
test("remarkMessageLinks: two URLs in one text node both replaced", () => {
  const tree = runPlugin(
    paragraph(
      text(
        "first g6://message?channel=a&id=1 then g6://message?channel=b&id=2 done",
      ),
    ),
  );
  const kids = tree.children[0].children;
  ...
  assert.equal(links[0].value, "g6://message?channel=a&id=1");
  assert.equal(links[1].value, "g6://message?channel=b&id=2");
});

test("remarkMessageLinks: trailing sentence punctuation stays outside URL", () => {
  for (const punctuation of [".", ",", ";", ":", "!", "?"]) {
    const tree = runPlugin(
      paragraph(text(`see g6://message?channel=c&id=m${punctuation}`)),
    );
    const kids = tree.children[0].children;
    ...
    assert.equal(kids[1].value, "g6://message?channel=c&id=m", punctuation);
...
test("remarkMessageLinks: URL inside parens keeps closing paren outside", () => {
  const tree = runPlugin(
    paragraph(text("see (g6://message?channel=c&id=m) for details")),
  );
  const kids = tree.children[0].children;
...
test("remarkMessageLinks: non-message g6:// URLs are not matched", () => {
  const original = "g6://connect?relay=wss://x.example";
...
test("remarkMessageLinks: text inside inlineCode is left alone", () => {
    ...
    children: [
      {
        type: "paragraph",
        children: [
          { type: "inlineCode", value: "g6://message?channel=c&id=m" },
        ],
      },
    ],
  };
  ...
  assert.equal(kids[0].value, "g6://message?channel=c&id=m");
});
...
function nudgeBody(agentPubkey) {
  return [
    ...
    "",
    "Open Edit Agent in the Gear6 app to set these.",
    "",
    "```g6:config-nudge",
    JSON.stringify({
      agent_name: "Fizz",
