...
function ImageBlock({ alt, dim, resolvedSrc, src, thumbSrc }: ImageBlockProps) {
  ...
  React.useLayoutEffect(() => {
    ...
    const spoiler = trigger.closest(".g6-spoiler[data-spoiler]");
    if (!spoiler) return;
 ...
 *
 * Gear6 renders inside a native webview whose default context menu has no
...
function createMarkdownComponents(
  ...
  function MarkdownAnchor({
    ...
    // Intercept `g6://message?channel=…&id=…` links so a click navigates
  ...
  return {
    ...
    "message-link": function MarkdownMessageLink({
      ...
      if (!parsed.ok) {
        // Malformed `g6://message?…` — render the raw URL as plain text
