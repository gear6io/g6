...
/**
 * Payload emitted by the Rust deep-link handler for `g6://message?…`.
...
/**
 * Payload emitted by the Rust deep-link handler for `g6://join?…` —
 ...
 *
 * When a `g6://connect?relay=<url>` link is opened, the handler
 ...
 *
 * When a `g6://join?relay=<url>&code=<invite>` link is opened (relay
 ...
 *
 * `g6://message?…` is handled separately by `listenForMessageDeepLinks`,
...
export function listenForMessageDeepLinks(
  ...
  if (USE_HTTP_API) return Promise.resolve(() => {});
