...
/**
 * Gear6 theme name. Gear6 is a first-party light theme that reuses GitHub
 ...
 * applied by {@link ThemeProvider} toggling a `data-g6-sidebar` attribute
 * on the document root; the CSS lives in `shared/styles/globals/theme.css`.
 */
export const GEAR6_THEME_NAME = "g6";

/**
 * Gear6 Dark theme name. The dark-mode counterpart to {@link GEAR6_THEME_NAME}:
 ...
 * {@link ThemeProvider} toggles the shared `data-g6-sidebar` attribute for
 * this theme too; the `.dark` root class selects the dark gradient values.
 *
 * Gear6 and Gear6 Dark are paired in {@link THEME_PAIRS}, so the picker shows a
 * combined "Gear6" tile under System mode (follow-OS) plus a single "Gear6" tile
 * under Light and a "Gear6 Dark" tile under Dark.
 */
export const GEAR6_DARK_THEME_NAME = "g6-dark";

/** The Shiki bundle Gear6 borrows its base palette from. */
export const GEAR6_BASE_THEME: SyntaxThemeName = "github-light";

/** The Shiki bundle Gear6 Dark borrows its base palette from. */
export const GEAR6_DARK_BASE_THEME: SyntaxThemeName = "github-dark";

/**
 * Resolve a theme name to the real Shiki bundled theme it maps to.
 *
 * Most themes map to themselves, but the Gear6 aliases (`g6` / `g6-dark`)
 ...
 * first; passing a raw Gear6 alias makes Shiki throw and code blocks fall
 * back to unhighlighted plain text.
 */
export function resolveShikiThemeName(name: string): SyntaxThemeName {
  if (name === GEAR6_THEME_NAME) return GEAR6_BASE_THEME;
  if (name === GEAR6_DARK_THEME_NAME) return GEAR6_DARK_BASE_THEME;
  return name as SyntaxThemeName;
}

// Available themes. "g6" is a Gear6-branded theme that reuses the
...
> = {
  // Gear6 reuses the github-light palette; its gradient is applied separately.
  g6: () => import("shiki/themes/github-light.mjs"),
  // Gear6 Dark reuses the github-dark palette; dark gradient applied separately.
  "g6-dark": () => import("shiki/themes/github-dark.mjs"),
...
export const THEME_PAIRS: ReadonlyMap<SyntaxThemeName, SyntaxThemeName> =
  new Map([
    // Light → Dark
    // Gear6 is the first-party pair; keep it first so it leads every category.
    ["g6", "g6-dark"],
