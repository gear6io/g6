...
test("malformed persisted state is ignored and can be cleared", () => {
  const storage = createMemoryStorage({
    "g6-community-onboarding-transaction.v1": '{"stage":"profile"}',
  });
...
test("completion is scoped by relay and pubkey and preserves legacy gate", () => {
  ...
  assert.equal(
    storage.getItem(
      "g6-community-onboarding-complete.v1:wss%3A%2F%2Frelay.example:pubkey",
    ),
    "true",
  );
  assert.equal(storage.getItem("g6-onboarding-complete.v1:pubkey"), "true");
});
