...
test("resolveAgentReadiness_cli_ignores_g6_agent_runtime", () => {
  // g6-agent with availability=available and logged_in must NOT trigger the CLI path.
  const runtimes = [
    makeRuntime({
      id: "g6-agent",
      label: "g6-agent",
...
test("resolveAgentReadiness_g6_agent_ready_when_provider_model_and_key_set", () => {
  // anthropic requires ANTHROPIC_API_KEY
  const result = resolveAgentReadiness(
    [makeRuntime({ id: "g6-agent", label: "Gear6 Agent" })],
    makeConfig({
  ...
  assert.deepEqual(result, { ready: true, reason: "g6-agent" });
});

test("resolveAgentReadiness_g6_agent_not_ready_when_missing_required_credential_key", () => {
...
test("resolveAgentReadiness_g6_agent_not_ready_when_provider_missing", () => {
  const config = makeConfig({
    provider: null,
    model: "claude-3-5-sonnet-latest",
    env_vars: { ANTHROPIC_API_KEY: "sk-ant-test" },
...
test("resolveAgentReadiness_g6_agent_not_ready_when_model_missing", () => {
  const config = makeConfig({
    provider: "anthropic",
    model: null,
    env_vars: { ANTHROPIC_API_KEY: "sk-ant-test" },
...
test("resolveAgentReadiness_legacy_config_without_preference_uses_g6_agent_fields", () => {
  const runtimes = [makeRuntime({ id: "g6-agent", label: "Gear6 Agent" })];
  const result = resolveAgentReadiness(
    runtimes,
    makeConfig({
...
test("resolveAgentReadiness_preferred_goose_does_not_borrow_ready_g6_agent_config", () => {
  const runtimes = [
    makeRuntime({ id: "goose", label: "Goose" }),
    makeRuntime({ id: "g6-agent", label: "Gear6 Agent" }),
  ];
