 ...
 * light-blue label (`--g6-onboarding-cta-label`, available on any
 * `.g6-onboarding-neutral-theme` subtree).
 */
export const ONBOARDING_PRIMARY_CTA_CLASS = `${ONBOARDING_CTA_SHAPE} text-[var(--g6-onboarding-cta-label)]`;

/**
 * Primary-CTA styling for the landing screen only: the shared pill with the
 * chartreuse label (`--g6-welcome-chartreuse`). The blue label is reserved
 * for the CTAs inside the steps.
 */
export const ONBOARDING_LANDING_CTA_CLASS = `${ONBOARDING_CTA_SHAPE} text-[var(--g6-welcome-chartreuse)]`;

/**
 * Icon-control styling for onboarding surfaces that sit on the textured card:
 * olive backup ink (`--g6-onboarding-backup-ink`) with a plain
...
export const ONBOARDING_INK_ICON_CLASS =
  "text-[color:var(--g6-onboarding-backup-ink)] hover:bg-transparent hover:text-foreground";

/**
 * Shared onboarding chrome shown on every page after the landing screen: a
 * static Gear6 mark pinned to the top-left, and a centered pagination track that
