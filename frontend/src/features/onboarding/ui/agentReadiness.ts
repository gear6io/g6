 ...
 * Provider path: the preferred Gear6 Agent or Goose runtime has provider and
...
export function resolveAgentReadiness(
  ...
  if (scope === "any") {
    for (const runtime of runtimes) {
      if (runtime.id === "g6-agent") continue;
      if (
  ...
  const preferredRuntime =
    scope === "preferred"
      ...
      : runtimes.find((runtime) => runtime.id === "g6-agent");
  ...
  if (preferredRuntime.id !== "g6-agent" && preferredRuntime.id !== "goose") {
    return { ready: false };
  }
  ...
  if (provider.length > 0 && model.length > 0) {
    ...
    if (allKeysPresent) {
      return { ready: true, reason: "g6-agent" };
    }
  }
