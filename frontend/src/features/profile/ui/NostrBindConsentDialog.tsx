...
const COPY_SUCCESS_MESSAGE =
  "Signed response copied. Paste it into the Gear6 admin console.";
const PREVIEW_COPY_SUCCESS_MESSAGE = "Preview response copied.";
const COPY_FAILURE_MESSAGE = "Gear6 couldn't access the clipboard. Try again.";
...
function appendCallbackStatus(callbackUrl: string): string {
  ...
  url.searchParams.set("g6_bind", "signed");
  return url.toString();
}
    ...
    const unlistenPromise = listenForNostrBindDeepLinks((nextPayload) => {
      ...
      getIdentity()
        ...
        .catch((error) => {
          ...
          setIdentity(null);
          setError("Could not load the current Gear6 identity.");
        });
    });
      ...
      <DialogPrimitive.Portal>
        {payload ? (
          <DialogPrimitive.Content
            ...
            className="g6-onboarding-neutral-theme g6-startup-shell fixed inset-0 z-50 flex overflow-y-auto bg-background px-4 py-12 text-foreground outline-hidden"
            ...
            <div className="m-auto flex w-full max-w-[500px] flex-col items-center text-center">
              ...
              {signedResponse ? (
                ...
                >
                  ...
                  >
                    {payload.returnMode === "browser_fragment_v1"
                      ? "Gear6 opened your browser to finish verification."
                      : "Copy the response below, then paste it into the Gear6 website to finish verification."}
                  </DialogPrimitive.Description>
