...
const DESKTOP_NOTIFICATION_ACTION_EVENT = "g6:desktop-notification-action";
...
type TestWindow = Window & {
  ...
  __GEAR6_E2E_APP_BADGE_STATE__?: AppBadgeState["kind"];
};
    ...
    try {
      pluginListener = await onAction((notification) => {
        const target = parseNotificationTarget(
          notification.extra?.g6NotificationTarget,
        );
        if (!target) {
          return;
        }
...
export async function setDesktopAppBadge(state: AppBadgeState): Promise<void> {
  if (typeof window !== "undefined") {
    ...
    testWindow.__GEAR6_E2E_APP_BADGE_COUNT__ =
      state.kind === "count" ? state.count : 0;
    testWindow.__GEAR6_E2E_APP_BADGE_STATE__ = state.kind;
  }
