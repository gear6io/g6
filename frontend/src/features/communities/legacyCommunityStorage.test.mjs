...
test("applyLegacyCommunityStorage seeds missing communities and active community", () => {
  ...
  assert.equal(storage.getItem("g6-communities"), legacyCommunities);
  assert.equal(storage.getItem("g6-active-community-id"), "legacy-community");
});

test("applyLegacyCommunityStorage preserves existing non-local Gear6 communities", () => {
  const storage = createMemoryStorage({
    "g6-communities": currentCommunities,
    "g6-active-community-id": "current-community",
  });
  ...
  assert.equal(storage.getItem("g6-communities"), currentCommunities);
  assert.equal(
    storage.getItem("g6-active-community-id"),
    "current-community",
  );
});
...
test("applyLegacyCommunityStorage replaces broken localhost first-run community", () => {
  const storage = createMemoryStorage({
    "g6-communities": localhostCommunities,
    "g6-active-community-id": "local-community",
  });
...
test("applyLegacyCommunityStorage migrates onboarding completion keys", () => {
  ...
  assert.equal(storage.getItem("g6-onboarding-complete.v1:abc123"), "true");
});
