/**
 * `g6://message` link encoding for "Copy link" / deep-link-to-message.
 *
 * Format: `g6://message?channel=<uuid>&id=<eventId>[&thread=<rootId>]`
 */

const MESSAGE_LINK_SCHEME = "g6:";
...
/**
 * Build a `g6://message` URL for a given channel + message.
 *
...
/**
 * Parse a `g6://message?…` URL. Returns a discriminated result so callers can
...
export function parseMessageLink(url: string): MessageLinkParseResult {
  ...
  // `new URL("g6://message?…")` puts "message" in `hostname`.
...
export function isMessageLink(href: string | undefined | null): boolean {
  ...
  return href.startsWith("g6://message?") || href === "g6://message";
}
 ...
 * CommonMark autolinks (`<g6://message?...>`) and explicitly labeled links
