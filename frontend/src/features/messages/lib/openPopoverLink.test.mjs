...
test("g6://message deep-link routes in-app, not the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink(`g6://message?channel=${CHANNEL}&id=${MESSAGE}`, handlers);
...
test("non-message g6:// URLs fall through to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  openPopoverLink("g6://channel?foo=bar", handlers);
  assert.deepEqual(external, ["g6://channel?foo=bar"]);
  assert.equal(inApp.length, 0);
});

test("malformed g6://message URL falls back to the OS opener", () => {
  const { handlers, external, inApp } = makeSpies();
  // Matches isMessageLink (starts with g6://message?) but is missing the
  // required channel/id params, so parse fails and we don't navigate in-app.
  openPopoverLink("g6://message?nope=1", handlers);
  assert.deepEqual(external, ["g6://message?nope=1"]);
  assert.equal(inApp.length, 0);
});
