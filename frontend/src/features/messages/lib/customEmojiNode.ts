...
export function registerCustomEmojiMarkdownIt(
  ...
  const RULE_NAME = "g6_custom_emoji";
  const TOKEN_TYPE = "g6_custom_emoji";
  ...
  const rule = (state: any, silent: boolean): boolean => {
    ...
    // word char. This keeps prose and URLs intact — `not:g6:` and
    // `http://x:y:g6:` must NOT turn the inner `:g6:` into an image;
