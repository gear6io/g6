...
test("buildMessageLink → parseMessageLink round-trips without thread", () => {
  ...
  assert.equal(url, `g6://message?channel=${CHANNEL}&id=${MESSAGE}`);
...
test("buildMessageLink treats null/empty thread as absent", () => {
  ...
  assert.equal(a, `g6://message?channel=${CHANNEL}&id=${MESSAGE}`);
  assert.equal(b, `g6://message?channel=${CHANNEL}&id=${MESSAGE}`);
});
...
test("parseMessageLink rejects g6:// with wrong host", () => {
  const r = parseMessageLink(`g6://connect?relay=wss://example.com`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "wrong-host");
});

test("parseMessageLink rejects missing channel", () => {
  const r = parseMessageLink(`g6://message?id=${MESSAGE}`);
  assert.equal(r.ok, false);
  assert.equal(r.ok === false && r.reason, "missing-channel");
});

test("parseMessageLink rejects missing id", () => {
  const r = parseMessageLink(`g6://message?channel=${CHANNEL}`);
  assert.equal(r.ok, false);
...
test("parseMessageLink accepts legacy g6://message links", () => {
  const r = parseMessageLink(`g6://message?channel=${CHANNEL}&id=${MESSAGE}`);
...
test("isMessageLink matches g6://message and legacy g6://message", () => {
  assert.equal(
    isMessageLink(`g6://message?channel=${CHANNEL}&id=${MESSAGE}`),
    true,
  );
  assert.equal(
    isMessageLink(`g6://message?channel=${CHANNEL}&id=${MESSAGE}`),
  ...
  assert.equal(isMessageLink("g6://connect?relay=wss://x"), false);
  assert.equal(isMessageLink("g6://connect?relay=wss://x"), false);
...
test("resolveMessageLinkRenderTarget distinguishes autolinks from labeled links", () => {
  const href = `g6://message?channel=${CHANNEL}&id=${MESSAGE}`;
