...
test("rule fires for a boundary :shortcode: (start of string)", () => {
  const rule = captureRule(["g6"]);
  const { matched, advanced } = runRule(rule, ":g6:", 0);
  assert.equal(matched, true);
  assert.equal(advanced, ":g6:".length);
});

test("rule fires for a :shortcode: preceded by whitespace", () => {
  const rule = captureRule(["g6"]);
  // pos points at the `:` after the space.
  const { matched } = runRule(rule, "hi :g6:", 3);
  assert.equal(matched, true);
});

test("rule does NOT fire when the colon is glued to a word char (not:g6:)", () => {
  const rule = captureRule(["g6"]);
  // pos points at the `:` immediately after `not`.
  const { matched } = runRule(rule, "not:g6:", 3);
  assert.equal(matched, false);
});

test("rule does NOT fire inside a URL-like sequence (http://x:y:g6:)", () => {
  const rule = captureRule(["g6"]);
  const src = "http://x:y:g6:";
  // pos points at the `:` immediately after `y` (a word char).
  const { matched } = runRule(rule, src, src.indexOf(":g6:"));
  assert.equal(matched, false);
});
...
test("rule fires after punctuation boundary (e.g. parenthesis)", () => {
  ...
  const { matched } = runRule(rule, "(:g6:)", 1);
  assert.equal(matched, true);
});
