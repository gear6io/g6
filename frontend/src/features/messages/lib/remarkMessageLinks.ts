/**
 * Remark plugin that detects bare `g6://message?…` URLs in text nodes and
 * replaces each with a custom `message-link` HAST element. Legacy
 * `g6://message?…` URLs are accepted during the rename. The `markdown.tsx`
 ...
 * and `www.`. Custom schemes like `g6://` only reach the `<a>` component
 * override when the user wrote an explicit `[label](g6://…)` link.
 *
...
const MESSAGE_URL_PATTERN = /(?:g6|g6):\/\/message\?[^\s<>"')\]]+/g;
