...
test("Gear6 Agent exposes provider, model, and Gear6-owned effort", () => {
  const model = deriveAgentConfigFieldModel({
    config,
    runtime: runtime("g6-agent", {
      modelEnvVar: "GEAR6_AGENT_MODEL",
      providerEnvVar: "GEAR6_AGENT_PROVIDER",
      thinkingEnvVar: "GEAR6_AGENT_THINKING_EFFORT",
    }),
    scope: "global",
  });
  ...
  assert.equal(field(model, "effort").optionSource, "g6AgentCatalog");
...
test("catalog mismatch cleanup is named and restricted to onboarding", () => {
  const selectedRuntime = runtime("g6-agent", {
    modelEnvVar: "GEAR6_AGENT_MODEL",
    providerEnvVar: "GEAR6_AGENT_PROVIDER",
    thinkingEnvVar: "GEAR6_AGENT_THINKING_EFFORT",
  });
