 ...
 *   g6-agent — classifies LLM failures into `AgentError` variants with
 *                  JSON-RPC codes (`-32001` auth, `-32002` model-not-found,
 *                  `-32000` generic), defined in `crates/g6-agent/src/types.rs`.
 *   g6-acp   — preserves the code structurally in
...
export function friendlyAgentLastError(
  ...
  // Match either the unwrapped g6-agent prefix or the g6-acp v0 wrap.
  if (
