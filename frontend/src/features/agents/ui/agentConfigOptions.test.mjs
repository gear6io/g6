...
test("getPersonaProviderOptions returns databricks v1 and v2 when hideProviderIds is empty", () => {
  const options = getPersonaProviderOptions("", "g6-agent", "", new Set());
...
test("getPersonaProviderOptions with no hideProviderIds omits the tail for a known provider", () => {
  const options = getPersonaProviderOptions("anthropic", "g6-agent");
  const tail = options.at(-1);
...
test("getDefaultPersonaRuntime ignores an unavailable global preference", () => {
  ...
  assert.equal(getDefaultPersonaRuntime(runtimes, "claude")?.id, "g6-agent");
});

test("getDefaultPersonaRuntime returns g6-agent over goose when both are available", () => {
  const runtimes = [
    makeRuntime("goose"),
    makeRuntime("g6-agent"),
    makeRuntime("claude"),
  ...
  assert.equal(result?.id, "g6-agent");
});

test("getDefaultPersonaRuntime falls back to goose when g6-agent is unavailable", () => {
  const runtimes = [
    makeRuntime("g6-agent", "not_installed"),
    makeRuntime("goose"),
  ];
  const result = getDefaultPersonaRuntime(runtimes);
...
test("getDefaultPersonaRuntime returns first available when neither g6-agent nor goose is available", () => {
  const runtimes = [
    makeRuntime("g6-agent", "adapter_missing"),
...
test("runtimeSupportsLlmProviderSelection is true for g6-agent and goose", () => {
  assert.equal(runtimeSupportsLlmProviderSelection("g6-agent"), true);
...
test("resetConfigForHarnessChange clears harness-specific values", () => {
  const config = {
    env_vars: { GEAR6_AGENT_THINKING_EFFORT: "high", KEEP_ME: "yes" },
    model: "claude-opus",
    preferred_runtime: "g6-agent",
    provider: "anthropic",
  };
...
test("getPersonaModelOptions for g6-agent with anthropic filters out zero-value default", () => {
  // anthropic requires explicit model — zero-value option is filtered out
  const options = getPersonaModelOptions("g6-agent", "anthropic");
...
test("getPersonaModelOptions for g6-agent with no provider returns default model", () => {
  const options = getPersonaModelOptions("g6-agent", "");
