/**
 * Source-of-truth constants for g6-agent model-tuning configuration knobs.
 *
 * Values must stay in sync with `crates/g6-agent/src/config.rs`
 * `parse_thinking_effort` — that function is the authoritative list.
 */

/** Env var key for the thinking/effort level sent to the LLM. */
export const GEAR6_AGENT_THINKING_EFFORT = "GEAR6_AGENT_THINKING_EFFORT";

/** Env var key for the maximum output token count per turn. */
export const GEAR6_AGENT_MAX_OUTPUT_TOKENS = "GEAR6_AGENT_MAX_OUTPUT_TOKENS";

/** Env var key for the context window token limit. */
export const GEAR6_AGENT_MAX_CONTEXT_TOKENS = "GEAR6_AGENT_MAX_CONTEXT_TOKENS";

/** Env var key for the maximum number of LLM/tool rounds per turn. */
export const GEAR6_AGENT_MAX_ROUNDS = "GEAR6_AGENT_MAX_ROUNDS";

/**
 * Ordered set of valid thinking-effort values accepted by g6-agent.
 * Mirrors `parse_thinking_effort` in `crates/g6-agent/src/config.rs`.
 */
export const GEAR6_AGENT_THINKING_EFFORT_VALUES = [
  "none",
  "minimal",
  "low",
  "medium",
...
export type ThinkingEffortValue =
  (typeof GEAR6_AGENT_THINKING_EFFORT_VALUES)[number];
 ...
 *
 * Mirrors the model-family tables in `crates/g6-agent/src/config.rs`
...
const ALL_VALUES = GEAR6_AGENT_THINKING_EFFORT_VALUES;
...
/**
 * Returns true when the given runtime id is g6-agent, which is the only
 * runtime that supports the tier-1 model-tuning knobs above.
 */
export function isGear6AgentRuntime(runtimeId: string): boolean {
  return runtimeId === "g6-agent";
}
