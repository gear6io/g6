...
function activityTitle(item) {
  return formatToolTitle(item.g6ToolName ?? item.toolName, item.title);
}
...
test("buildTranscript renders Prompt context + user message for a multi-block session/prompt frame", () => {
  ...
  assert.deepEqual(
    ...
    ["Agent Memory — core", "Context", "Gear6 event: @mention"],
...
test("buildTranscript keeps read_file activity categorized by the actual tool when output names Gear6 tools", () => {
  const [item] = toolItems([
    ...
    acpToolUpdate(11, {
      ...
      content: {
        type: "text",
        text: 'const GEAR6_READ_TOOLS = new Set(["get_feed", "get_event"]);\nconst GEAR6_WRITE_TOOLS = new Set(["delete_message"]);',
      },
    }),
  ]);

  assert.equal(item.toolName, "read_file");
  assert.equal(item.g6ToolName, null);
...
test("buildTranscript keeps shell activity categorized by the actual tool when grep output names Gear6 tools", () => {
  const [item] = toolItems([
    acpToolUpdate(20, {
      sessionUpdate: "tool_call",
...
test("buildTranscript categorizes explicit Gear6 tool calls for the activity bar", () => {
  const [item] = toolItems([
    acpToolUpdate(30, {
      sessionUpdate: "tool_call",
  ...
  assert.equal(item.g6ToolName, "get_feed");
...
test("buildTranscript five-section system prompt card is standalone with all sections; CheckCheck context contains only Gear6/thread context", () => {
  ...
  //       the session/prompt:context sections (Gear6 event + Thread context),
      ...
      payload: {
        ...
        params: {
          systemPrompt: [
            ...
            "Fetch current content with: g6 canvas get --channel 44444444-4444-4444-4444-444444444444",
          ].join("\n"),
        },
      },
    },
  ...
  // the session/prompt:context item — Gear6/thread context only, no system-prompt sections.
  ...
  // Must have Gear6 event and Thread context sections, NOT Base/System/Team Instructions/Core Memory/Channel Canvas.
  assert.ok(
    contextSectionTitles.some((t) => t.toLowerCase().includes("gear6")),
    "prompt context must contain a Gear6 event section",
  );
  assert.ok(
