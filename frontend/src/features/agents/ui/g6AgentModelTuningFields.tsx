/**
 * Tier-1 g6-agent model-tuning UI fields.
 *
...
/**
 * Shared effort-select dropdown for the `GEAR6_AGENT_THINKING_EFFORT` env var.
 *
 * Used by both `Gear6AgentModelTuningFields` (per-agent/persona dialogs) and
...
export function EffortSelectField({
   ...
   * Defaults to `"Inherit"`.
   *
   * Per-agent callers (Gear6AgentModelTuningFields) pass `"Inherit (agent default)"`
  ...
  const effortOptions: AgentDropdownOption[] = [
    ...
    ...GEAR6_AGENT_THINKING_EFFORT_VALUES.flatMap((v) => {
...
/**
 * Auto-clear hook: resets `GEAR6_AGENT_THINKING_EFFORT` to "" (Inherit) when
...
export function Gear6AgentModelTuningFields({
  envVars,
  inheritedEnvVars,
  model,
  onEnvVarChange,
  ...
  const currentEffort = envVars[GEAR6_AGENT_THINKING_EFFORT] ?? "";
  useEffortAutoClear({
    currentEffort,
    effortValid,
    onClear: () => onEnvVarChange(GEAR6_AGENT_THINKING_EFFORT, ""),
  });

  return (
    <div className="space-y-4">
      <p className="text-2xs font-semibold uppercase tracking-wide text-muted-foreground">
        g6-agent model tuning
      </p>
      ...
      <div className="grid gap-4 md:grid-cols-2">
        ...
        <div className="space-y-1.5">
          <EffortSelectField
            ...
            inheritedEffort={inheritedEnvVars[GEAR6_AGENT_THINKING_EFFORT]}
            inheritFallbackLabel="Inherit (agent default)"
            label="Thinking / Effort"
            onChange={(value) =>
              onEnvVarChange(GEAR6_AGENT_THINKING_EFFORT, value)
            }
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            min="0"
            onChange={(event) =>
              onEnvVarChange(GEAR6_AGENT_MAX_ROUNDS, event.target.value)
            }
            placeholder={
              inheritedEnvVars[GEAR6_AGENT_MAX_ROUNDS]
                ? `Inherit (${inheritedEnvVars[GEAR6_AGENT_MAX_ROUNDS]})`
                : "Inherit (agent default)"
            }
            step="1"
            type="number"
            value={envVars[GEAR6_AGENT_MAX_ROUNDS] ?? ""}
          />
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            min="1"
            onChange={(event) =>
              onEnvVarChange(GEAR6_AGENT_MAX_OUTPUT_TOKENS, event.target.value)
            }
            placeholder={
              inheritedEnvVars[GEAR6_AGENT_MAX_OUTPUT_TOKENS]
                ? `Inherit (${inheritedEnvVars[GEAR6_AGENT_MAX_OUTPUT_TOKENS]})`
                : "Inherit (agent default)"
            }
            step="1"
            type="number"
            value={envVars[GEAR6_AGENT_MAX_OUTPUT_TOKENS] ?? ""}
          />
          <p
            className="text-xs text-muted-foreground"
        ...
        <div className="space-y-1.5">
          ...
          <Input
            ...
            min="1"
            onChange={(event) =>
              onEnvVarChange(GEAR6_AGENT_MAX_CONTEXT_TOKENS, event.target.value)
            }
            placeholder={
              inheritedEnvVars[GEAR6_AGENT_MAX_CONTEXT_TOKENS]
                ? `Inherit (${inheritedEnvVars[GEAR6_AGENT_MAX_CONTEXT_TOKENS]})`
                : "Inherit (agent default)"
            }
            step="1"
            type="number"
            value={envVars[GEAR6_AGENT_MAX_CONTEXT_TOKENS] ?? ""}
          />
          <p
            className="text-xs text-muted-foreground"
            id="help-ba-max-context-tokens"
          >
            Maximum context window tokens g6-agent tracks before a handoff.
