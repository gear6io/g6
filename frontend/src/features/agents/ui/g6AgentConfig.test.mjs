...
import {
  GEAR6_AGENT_MAX_CONTEXT_TOKENS,
  GEAR6_AGENT_MAX_OUTPUT_TOKENS,
  GEAR6_AGENT_MAX_ROUNDS,
  GEAR6_AGENT_THINKING_EFFORT,
  GEAR6_AGENT_THINKING_EFFORT_VALUES,
  getProviderEffortConfig,
  isGear6AgentRuntime,
} from "./g6AgentConfig.ts";

// ---------------------------------------------------------------------------
// Thinking effort values
// ---------------------------------------------------------------------------

test("GEAR6_AGENT_THINKING_EFFORT_VALUES contains exactly the 7 accepted values", () => {
  assert.deepEqual(
    [...GEAR6_AGENT_THINKING_EFFORT_VALUES],
...
test("GEAR6_AGENT_THINKING_EFFORT_VALUES has no duplicates", () => {
  const set = new Set(GEAR6_AGENT_THINKING_EFFORT_VALUES);
  assert.equal(set.size, GEAR6_AGENT_THINKING_EFFORT_VALUES.length);
});
...
test("env var key constants match expected GEAR6_AGENT_* names", () => {
  assert.equal(GEAR6_AGENT_THINKING_EFFORT, "GEAR6_AGENT_THINKING_EFFORT");
  assert.equal(GEAR6_AGENT_MAX_OUTPUT_TOKENS, "GEAR6_AGENT_MAX_OUTPUT_TOKENS");
  assert.equal(GEAR6_AGENT_MAX_CONTEXT_TOKENS, "GEAR6_AGENT_MAX_CONTEXT_TOKENS");
  assert.equal(GEAR6_AGENT_MAX_ROUNDS, "GEAR6_AGENT_MAX_ROUNDS");
});
...
test("isGear6AgentRuntime returns true only for g6-agent id", () => {
  assert.equal(isGear6AgentRuntime("g6-agent"), true);
});

test("isGear6AgentRuntime returns false for other runtimes", () => {
  assert.equal(isGear6AgentRuntime("goose"), false);
  assert.equal(isGear6AgentRuntime("custom"), false);
  assert.equal(isGear6AgentRuntime(""), false);
  assert.equal(isGear6AgentRuntime("g6-agent-v2"), false);
});
...
test("setting a thinking effort value writes the key into envVars", () => {
  const initial = {};
  const result = applyEnvVarChange(initial, GEAR6_AGENT_THINKING_EFFORT, "high");
  assert.equal(result[GEAR6_AGENT_THINKING_EFFORT], "high");
});

test("clearing thinking effort removes the key so the agent inherits", () => {
  const initial = { [GEAR6_AGENT_THINKING_EFFORT]: "high" };
  const result = applyEnvVarChange(initial, GEAR6_AGENT_THINKING_EFFORT, "");
  assert.equal(Object.hasOwn(result, GEAR6_AGENT_THINKING_EFFORT), false);
});

test("setting max output tokens writes the exact GEAR6_AGENT_MAX_OUTPUT_TOKENS key", () => {
  const initial = {};
  ...
  assert.equal(result[GEAR6_AGENT_MAX_OUTPUT_TOKENS], "4096");
  // Must not affect other keys
  assert.equal(Object.keys(result).length, 1);
});

test("clearing max output tokens removes the key", () => {
  const initial = { [GEAR6_AGENT_MAX_OUTPUT_TOKENS]: "4096" };
  const result = applyEnvVarChange(initial, GEAR6_AGENT_MAX_OUTPUT_TOKENS, "");
  assert.equal(Object.hasOwn(result, GEAR6_AGENT_MAX_OUTPUT_TOKENS), false);
});

test("setting context limit writes the exact GEAR6_AGENT_MAX_CONTEXT_TOKENS key", () => {
  const initial = {};
  const result = applyEnvVarChange(
    initial,
    GEAR6_AGENT_MAX_CONTEXT_TOKENS,
  ...
  assert.equal(result[GEAR6_AGENT_MAX_CONTEXT_TOKENS], "100000");
});

test("clearing context limit removes the key", () => {
  const initial = { [GEAR6_AGENT_MAX_CONTEXT_TOKENS]: "100000" };
  const result = applyEnvVarChange(initial, GEAR6_AGENT_MAX_CONTEXT_TOKENS, "");
  assert.equal(Object.hasOwn(result, GEAR6_AGENT_MAX_CONTEXT_TOKENS), false);
});

test("setting max rounds writes the exact GEAR6_AGENT_MAX_ROUNDS key", () => {
  const initial = {};
  const result = applyEnvVarChange(initial, GEAR6_AGENT_MAX_ROUNDS, "50");
  assert.equal(result[GEAR6_AGENT_MAX_ROUNDS], "50");
});

test("clearing max rounds removes the key", () => {
  const initial = { [GEAR6_AGENT_MAX_ROUNDS]: "50" };
  const result = applyEnvVarChange(initial, GEAR6_AGENT_MAX_ROUNDS, "");
  assert.equal(Object.hasOwn(result, GEAR6_AGENT_MAX_ROUNDS), false);
});

test("changing one field does not disturb other env vars", () => {
  const initial = {
    SOME_OTHER_KEY: "value",
    [GEAR6_AGENT_MAX_OUTPUT_TOKENS]: "2048",
  };
  const result = applyEnvVarChange(initial, GEAR6_AGENT_MAX_ROUNDS, "20");
  assert.equal(result.SOME_OTHER_KEY, "value");
  assert.equal(result[GEAR6_AGENT_MAX_OUTPUT_TOKENS], "2048");
  assert.equal(result[GEAR6_AGENT_MAX_ROUNDS], "20");
});
...
test("thinking effort select is bounded: all 7 accepted values are present in the constant", () => {
  ...
  for (const v of expected) {
    assert.ok(
      GEAR6_AGENT_THINKING_EFFORT_VALUES.includes(v),
      `missing value: ${v}`,
    );
  }
  assert.equal(GEAR6_AGENT_THINKING_EFFORT_VALUES.length, expected.length);
});
...
test("non-numeric string is stored as-is (validation is at the backend)", () => {
  ...
  assert.equal(result[GEAR6_AGENT_MAX_OUTPUT_TOKENS], "not-a-number");
});
...
// Mirrors the `isGear6Agent` derivation in CreateAgentRuntimeFields.
...
test("isGear6AgentRuntime(prospectiveRuntimeId) shows fields when Edit resolves g6-agent even though selectedRuntimeId sentinel is 'inherit'", () => {
  // Simulates Edit dialog state: inheritHarness=true, persona is g6-agent.
  // selectedRuntimeId would be "inherit" (sentinel for custom-command hiding),
  // but prospectiveRuntimeId correctly resolves to "g6-agent".
  const selectedRuntimeIdSentinel = "inherit"; // what Edit passes to selectedRuntimeId
  const prospectiveRuntimeId = "g6-agent"; // what Edit passes to modelTuningRuntimeId

  assert.equal(
    isGear6AgentRuntime(selectedRuntimeIdSentinel),
    false,
    "sentinel 'inherit' must NOT trigger model-tuning fields",
  );
  assert.equal(
    isGear6AgentRuntime(prospectiveRuntimeId),
    true,
    "prospectiveRuntimeId 'g6-agent' MUST trigger model-tuning fields",
  );
});

test("isGear6AgentRuntime(prospectiveRuntimeId) shows fields when Edit has a pinned g6-agent (selectedRuntimeId sentinel is also 'inherit')", () => {
  // Simulates Edit dialog with a pinned non-custom runtime:
  // selectedRuntimeId sentinel = "inherit" (non-custom known runtime),
  // prospectiveRuntimeId = "g6-agent" (selectedRuntime?.id).
  const selectedRuntimeIdSentinel = "inherit";
  const prospectiveRuntimeId = "g6-agent";

  assert.equal(isGear6AgentRuntime(prospectiveRuntimeId), true);
  assert.equal(isGear6AgentRuntime(selectedRuntimeIdSentinel), false);
});

test("isGear6AgentRuntime(prospectiveRuntimeId) hides fields when Edit resolves to non-g6-agent", () => {
  // E.g. user switches from g6-agent to goose in Edit — prospectiveRuntimeId = "goose"
  const prospectiveRuntimeId = "goose";
  assert.equal(isGear6AgentRuntime(prospectiveRuntimeId), false);
});

test("isGear6AgentRuntime(prospectiveRuntimeId) hides fields when Edit has no resolved runtime (empty string)", () => {
...
// useEffect in Gear6AgentModelTuningFields and AgentDefaultsSettingsCard
