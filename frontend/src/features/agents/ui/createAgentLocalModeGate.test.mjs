...
test("localMode_g6Agent_supportsProviderSelection", () => {
  assert.equal(
    runtimeSupportsLlmProviderSelection("g6-agent"),
    true,
    "g6-agent must support LLM provider selection",
  );
});
...
test("localMode_g6Agent_emptyProvider_notSatisfied", () => {
  // Scenario: user selects g6-agent but leaves provider empty.
  // Rust readiness requires GEAR6_AGENT_PROVIDER — empty = NotReady.
...
test("localMode_g6Agent_emptyModel_notSatisfied", () => {
  // Scenario: g6-agent + anthropic + API key present, but model left empty.
  // Rust readiness requires GEAR6_AGENT_MODEL — empty = NotReady.
...
// ── Gate: g6-agent / anthropic with missing key → markers shown ─────────

test("localMode_g6Agent_anthropic_missingKey_notSatisfied", () => {
  // Scenario: user selects g6-agent/anthropic + fills model, but hasn't
...
test("localMode_g6Agent_anthropic_allRequired_present_allowed", () => {
...
test("localMode_requiredEnvKeys_surfaces_anthropicKey", () => {
  ...
  const allKeys = requiredCredentialEnvKeys("g6-agent", "anthropic");
  assert.ok(
    allKeys.includes("ANTHROPIC_API_KEY"),
    "requiredCredentialEnvKeys must include ANTHROPIC_API_KEY for g6-agent/anthropic",
  );
});
...
test("localMode_requiredEnvKeys_gate_and_envVarsEditor_share_same_key_set", () => {
  ...
  const fullKeys = requiredCredentialEnvKeys("g6-agent", "anthropic");
...
test("localMode_goose_providerSatisfiedByFileConfig_noNormalizedFieldRequired", () => {
  // Scenario: goose, no provider in Gear6 env but file config has provider + model.
...
test("providerDefaultLabel_noGlobal_returnsSelectAProvider", () => {
  ...
  const label = getDefaultLlmProviderLabel("g6-agent", undefined);
  assert.equal(
    label,
    "Select a provider\u2026",
...
test("providerDefaultLabel_emptyGlobal_returnsSelectAProvider", () => {
  ...
  const label = getDefaultLlmProviderLabel("g6-agent", "");
  assert.equal(
    label,
    "Select a provider\u2026",
...
test("providerDefaultLabel_globalSet_returnsInheritLabel", () => {
  ...
  const label = getDefaultLlmProviderLabel("g6-agent", "anthropic");
  assert.equal(
    label,
...
test("providerDefaultLabel_globalSetWithWhitespace_trimsAndReturnsInherit", () => {
  ...
  const label = getDefaultLlmProviderLabel("g6-agent", "  openai  ");
  assert.equal(
    label,
...
test("providerDefaultLabel_sharedCompute_neverLeaksInternalId", () => {
  assert.equal(
    getDefaultLlmProviderLabel("g6-agent", "relay-mesh"),
    "Use agent defaults (Gear6 shared compute)",
  );
});
...
test("bakedDefaults_emptyGlobal_usesBuildValuesForCreateAndEditLabels", () => {
  const bakedEnv = [
    { key: "GEAR6_AGENT_PROVIDER", value: "databricks_v2", masked: false },
    {
      key: "GEAR6_AGENT_MODEL",
      value: "goose-claude-opus-4-8",
      masked: false,
    },
    { key: "GEAR6_AGENT_THINKING_EFFORT", value: "high", masked: false },
  ];
  ...
  const model = resolveInheritedDefault(null, bakedEnv, "GEAR6_AGENT_MODEL");
  const effort = resolveInheritedDefault(
    null,
    bakedEnv,
    "GEAR6_AGENT_THINKING_EFFORT",
  );
  const providerOptions = getPersonaProviderOptions("", "g6-agent");
...
test("bakedDefaults_explicitGlobalsOverrideBuildValues", () => {
  ...
  assert.deepEqual(
    resolveInheritedDefault("anthropic", bakedEnv, "GEAR6_AGENT_PROVIDER"),
    { source: "global", value: "anthropic" },
  );
  assert.deepEqual(
    resolveInheritedDefault("global-model", bakedEnv, "GEAR6_AGENT_MODEL"),
    { source: "global", value: "global-model" },
  );
  assert.deepEqual(
    resolveInheritedDefault("low", bakedEnv, "GEAR6_AGENT_THINKING_EFFORT"),
...
test("f3b_buildTemplateModelDropdownOptions_anthropicGlobalModelSet_containsInheritOption", () => {
  ...
  const staticOptions = getPersonaModelOptions("g6-agent", "anthropic");
...
test("f3b_buildTemplateModelDropdownOptions_blankProviderGlobalModelSet_noDoubleSeed", () => {
  ...
  const staticOptions = getPersonaModelOptions("g6-agent", "");
...
// include ANTHROPIC_API_KEY for explicit g6-agent/anthropic so that the
...
test("providerConfig_explicitAnthropic_requiredKeysIncludesApiKey", () => {
  ...
  const required = requiredCredentialEnvKeys("g6-agent", "anthropic");
  assert.ok(
    required.includes("ANTHROPIC_API_KEY"),
    "g6-agent + explicit anthropic must list ANTHROPIC_API_KEY as required",
  );
});
...
test("providerConfig_inheritThenExplicitAnthropic_sameRequiredKeys", () => {
  ...
  const inheritKeys = requiredCredentialEnvKeys("g6-agent", "");
  const explicitKeys = requiredCredentialEnvKeys("g6-agent", "anthropic");
  assert.ok(
...
test("providerConfig_databricks_requiredKeyIsHost_noSecretClear", () => {
  ...
  const databricksKeys = requiredCredentialEnvKeys("g6-agent", "databricks");
  assert.ok(
    databricksKeys.includes("DATABRICKS_HOST"),
    "g6-agent + databricks must list DATABRICKS_HOST as required",
  );
