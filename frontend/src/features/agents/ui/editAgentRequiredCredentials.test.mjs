...
test("editAgent_globalProvider_globalApiKey_noPerAgentEnv_notMissing", () => {
  // Setup: g6-agent runtime, provider = anthropic (from global), key in globalEnvVars.
  ...
  assert.ok(
    ...
    "ANTHROPIC_API_KEY must appear in raw required keys for g6-agent/anthropic",
  );
...
test("editAgent_globalProvider_globalApiKeyEmpty_stillMissing", () => {
  // Setup: g6-agent runtime, provider = anthropic (from global), key NOT in globalEnvVars.
...
test("editAgent_globalSatisfiesOneKey_otherKeyStillMissing", () => {
  ...
  const allKeysOpenai = requiredCredentialEnvKeys("g6-agent", "openai");
  const allKeysAnthropic = requiredCredentialEnvKeys("g6-agent", "anthropic");
