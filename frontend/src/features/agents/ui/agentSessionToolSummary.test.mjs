...
test("buildCompactToolSummary formats Gear6 send_message preview", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "send_message",
      g6ToolName: "send_message",
      title: "Send Message",
...
test("buildCompactToolSummary treats g6 messages send commands as messages", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "g6-dev-mcp__shell",
      args: {
        command:
          'g6 --format compact messages send --channel channel-1 --content "@Ned are you working"',
      },
    }),
  );
...
test("buildCompactToolSummary returns null preview for piped stdin sends", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      ...
      args: {
        command:
          'echo "hello from stdin" | ./target/release/g6 messages send --channel channel-1 --content -',
      },
    }),
  );
...
test("buildCompactToolSummary formats view_image thumbnail source", () => {
  ...
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "g6-dev-mcp__view_image",
      args: { source },
    }),
  );
...
test("buildCompactToolSummary promotes non-send g6 CLI commands to relay ops", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "shell",
      args: {
        command: "g6 channels get --channel channel-1",
      },
    }),
  );
  ...
  assert.equal(summary.presentation, "inline");
  assert.equal(summary.shellContent, "g6 channels get --channel channel-1");
});

test("buildCompactToolSummary exposes shellContent for shell-sourced g6 CLI reads", () => {
  const command =
    "sleep 45; g6 messages thread --channel channel-uuid --event abc | tail -n 20";
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "shell",
      args: { command },
...
test("buildCompactToolSummary derives structured actions for native Gear6 MCP tools", () => {
  const summary = buildCompactToolSummary(
    makeTool({
      toolName: "get_channel",
      g6ToolName: "get_channel",
      args: {
