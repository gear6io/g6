...
test("storageKey: returns expected format with pubkey", () => {
  assert.equal(storageKey("abc123"), "g6-channel-sections.v1:abc123");
});
...
test("storageKey: with relayUrl includes normalized+encoded relay in key", () => {
  ...
  assert.equal(
    key,
    `g6-channel-sections.v1:pk1:${encodeURIComponent(normalizeRelayUrl(relay))}`,
  );
});

test("storageKey: without relayUrl returns legacy pubkey-only key", () => {
  assert.equal(storageKey("pk1"), "g6-channel-sections.v1:pk1");
  assert.equal(storageKey("pk1", undefined), "g6-channel-sections.v1:pk1");
});
