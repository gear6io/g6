...
fn reconcile_legacy_command_names_rewrites_renamed_sidecars() {
    ...
    assert_eq!(records[0]["acp_command"], "g6-acp");
    assert_eq!(records[0]["agent_command"], "g6-agent");
    assert_eq!(records[0]["mcp_command"], "g6-dev-mcp");
}

#[test]
fn reconcile_legacy_command_names_updates_removed_mcp_server_for_g6_agent() {
    let dir = tempfile::tempdir().unwrap();
    write_agents_json(
        dir.path(),
        &serde_json::json!([{
...
fn reconcile_legacy_command_names_rewrites_persona_runtime() {
    ...
    assert_eq!(records[0]["runtime"], "g6-agent");
}
...
fn rewrite_legacy_persona_md_runtime_rewrites_frontmatter_only() {
    ...
    assert!(updated.contains("runtime: g6-agent\n"));
