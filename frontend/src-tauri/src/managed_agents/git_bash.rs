//! Git Bash discovery shared by Doctor and the g6-agent readiness gate.
//!
...
//! Git-for-Windows registry. A Doctor green state therefore means `g6-dev-mcp`
...
/// (including all non-Windows platforms). Honors `GEAR6_SHELL` (any executable) —
...
///
/// Unlike `resolve_git_bash_path`, this skips `GEAR6_SHELL` entirely — that override
...
pub(crate) fn resolve_bash_path() -> Option<std::path::PathBuf> {
    #[cfg(windows)]
    {
        ...
        return resolve_git_bash(
            &env.path,
            None, // skip GEAR6_SHELL — install/login-shell callers require bash
            env.git_bash_override,
            env.system_root,
            env.program_files,
            env.program_files_x86,
...
/// deliberately sourced from `g6_agent_pkg::WINDOWS_SHELL_RESOLUTION_ENV`,
...
impl GitBashEnv {
    ...
    fn from_process_with_overrides(overrides: &std::collections::BTreeMap<String, String>) -> Self {
        let values: std::collections::HashMap<_, _> = g6_agent_pkg::WINDOWS_SHELL_RESOLUTION_ENV
            .iter()
            .filter_map(|key| {
                overrides
                    .iter()
    ...
    fn from_lookup(mut get: impl FnMut(&str) -> Option<std::ffi::OsString>) -> Self {
        Self {
            path: get("PATH")
                ...
                .to_string_lossy()
                .into_owned(),
            shell_override: get("GEAR6_SHELL").map(PathBuf::from),
...
/// Resolve `GEAR6_SHELL` with the same rooted/bare-name semantics as the MCP
...
mod tests {
    ...
    fn test_detector_env_keys_match_agent_shell_resolution_contract() {
        assert_eq!(
            DETECTOR_ENV_KEYS,
            g6_agent_pkg::WINDOWS_SHELL_RESOLUTION_ENV,
        ...
        assert_eq!(env.shell_override, Some(PathBuf::from("GEAR6_SHELL")));
    ...
    #[test]
    fn test_effective_g6_shell_override_marks_agent_ready() {
        let temp = tempdir().expect("tempdir");
        let shell = temp.path().join("pwsh.exe");
        std::fs::write(&shell, []).expect("shell");

        let mut overrides = std::collections::BTreeMap::new();
        overrides.insert("g6_shell".to_string(), shell.display().to_string());
        let env = GitBashEnv::from_process_with_overrides(&overrides);
    ...
    #[test]
    fn test_g6_shell_override_wins_over_git_bash_discovery() {
        let temp = tempdir().expect("tempdir");
        let shell = temp.path().join("pwsh.exe");
    ...
    // ── Regression: install/login-shell must skip non-bash GEAR6_SHELL ─────────

    /// When GEAR6_SHELL=pwsh.exe, `resolve_git_bash` with `shell_override=None`
    ...
    #[test]
    fn test_install_path_skips_g6_shell_pwsh() {
        let temp = tempdir().expect("tempdir");
        ...
        // Readiness gate: GEAR6_SHELL=pwsh accepted (Doctor green).
        assert_eq!(
            resolve_git_bash(path_str, Some(pwsh.clone()), None, None, None, None, None),
            Some(pwsh),
            "readiness gate must accept GEAR6_SHELL=pwsh"
        );
        ...
        assert_eq!(
            ...
            Some(bash),
            "install path must skip GEAR6_SHELL and find bash on PATH"
        );
    }

    /// Same as above but with GEAR6_SHELL=cmd.exe.
    #[test]
    fn test_install_path_skips_g6_shell_cmd() {
        let temp = tempdir().expect("tempdir");
        ...
        // Readiness gate: GEAR6_SHELL=cmd accepted.
        assert_eq!(
            resolve_git_bash(path_str, Some(cmd.clone()), None, None, None, None, None),
            Some(cmd),
            "readiness gate must accept GEAR6_SHELL=cmd"
        );
