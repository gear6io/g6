...
fn merged_env_strips_reserved_keys_from_persona() {
    ...
    let persona = map(&[
        ("GEAR6_PRIVATE_KEY", "nsec1evil"),
        ("ANTHROPIC_API_KEY", "ok"),
    ]);
    let merged = merged_user_env(&persona, &BTreeMap::new());
    assert!(!merged.contains_key("GEAR6_PRIVATE_KEY"));
    assert_eq!(
        merged.get("ANTHROPIC_API_KEY").map(String::as_str),
        Some("ok")
    );
...
fn merged_env_strips_reserved_keys_from_agent() {
    ...
    assert!(!merged.contains_key("GEAR6_AUTH_TAG"));
    assert_eq!(merged.get("FOO").map(String::as_str), Some("1"));
    assert_eq!(merged.len(), 1);
}
...
fn merged_env_strips_reserved_case_insensitive() {
    ...
    // refuse close-typo variants too — a lowercase `g6_private_key`
    // is almost certainly a footgun, not a legitimate use.
    let agent = map(&[("g6_private_key", "x"), ("Gear6_Auth_Tag", "y")]);
    let merged = merged_user_env(&BTreeMap::new(), &agent);
    assert!(merged.is_empty());
}
...
fn is_reserved_recognises_full_list() {
    ...
    assert!(!is_reserved_env_key("GEAR6_ACP_MODEL")); // behavior knob
}

#[test]
fn reserved_keys_include_agent_owner_for_legacy_records() {
    // Legacy records without auth_tag fall back to GEAR6_ACP_AGENT_OWNER
    // to enforce the respond-to gate. Must not be user-overridable.
    assert!(is_reserved_env_key("GEAR6_ACP_AGENT_OWNER"));
    let agent = map(&[("GEAR6_ACP_AGENT_OWNER", "imposter")]);
    let merged = merged_user_env(&BTreeMap::new(), &agent);
    assert!(merged.is_empty());
}
...
fn reserved_keys_include_respond_to_gate() {
    ...
    for key in ["GEAR6_ACP_RESPOND_TO", "GEAR6_ACP_RESPOND_TO_ALLOWLIST"] {
        assert!(is_reserved_env_key(key), "{key} should be reserved");
...
fn reserved_keys_include_code_execution_surface() {
    // The agent/MCP command + args are what Gear6 actually exec's.
...
fn reserved_keys_include_relay_url() {
    ...
    assert!(is_reserved_env_key("GEAR6_RELAY_URL"));
    let agent = map(&[("GEAR6_RELAY_URL", "ws://attacker.example")]);
    let merged = merged_user_env(&BTreeMap::new(), &agent);
    assert!(merged.is_empty());
}
...
fn validate_keys_rejects_reserved() {
    let env = map(&[("GEAR6_PRIVATE_KEY", "nsec1evil")]);
    let err = validate_user_env_keys(&env).unwrap_err();
    assert!(err.contains("GEAR6_PRIVATE_KEY"), "got: {err}");
...
fn validate_keys_lists_all_reserved_keys_found() {
    ...
    let err = validate_user_env_keys(&env).unwrap_err();
    assert!(err.contains("GEAR6_PRIVATE_KEY"));
...
// `c.env("GEAR6_AUTH_TAG=x", "forged")` results in the child seeing
// `GEAR6_AUTH_TAG=x=forged` and `getenv("GEAR6_AUTH_TAG") == "x=forged"`.
...
fn is_well_formed_rejects_malformed_keys() {
    for key in [
        "",                  // empty
        "=",                 // bare equals
        "GEAR6_AUTH_TAG=x",   // =-in-key bypass
        "GEAR6_PRIVATE_KEY=", // trailing equals
        "FOO BAR",           // space
...
fn validate_keys_rejects_equals_in_key_bypass() {
    // The actual exploit: `GEAR6_AUTH_TAG=x` smuggles a value past
    // the reserved-key string compare and into the child's environ.
    let env = map(&[("GEAR6_AUTH_TAG=x", "forged")]);
    let err = validate_user_env_keys(&env).unwrap_err();
    assert!(err.contains("[A-Za-z_]"), "got: {err}");
    ...
    assert!(err.contains("GEAR6_AUTH_TAG"), "got: {err}");
...
fn validate_keys_reports_malformed_before_reserved() {
    ...
    // from churning.
    let env = map(&[("GEAR6_AUTH_TAG=x", "v"), ("GEAR6_PRIVATE_KEY", "v")]);
    let err = validate_user_env_keys(&env).unwrap_err();
    assert!(err.contains("[A-Za-z_]"), "got: {err}");
    assert!(!err.contains("reserved"), "got: {err}");
}
...
fn merged_env_drops_malformed_keys() {
    ...
    let agent = map(&[
        ("GEAR6_AUTH_TAG=x", "forged"),
        ("FOO=bar", "v"),
        ("LEGIT", "ok"),
    ]);
    let merged = merged_user_env(&BTreeMap::new(), &agent);
    assert!(!merged.contains_key("GEAR6_AUTH_TAG=x"));
...
// GEAR6_AGENT_MODEL, GEAR6_AGENT_PROVIDER) so they don't shadow the
...
fn is_derived_key_is_case_insensitive() {
    ...
    assert!(is_derived_provider_model_key("g6_agent_model"));
    assert!(is_derived_provider_model_key("GEAR6_AGENT_PROVIDER"));
}
...
fn is_derived_key_does_not_match_unrelated_keys() {
    ...
    assert!(!is_derived_provider_model_key("GEAR6_PRIVATE_KEY"));
