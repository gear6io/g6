...
//!
//! Internal builds (g6-releases) bake arbitrary `KEY=VALUE` pairs into the
//! binary via `GEAR6_BUILD_AGENT_ENV` (base64-encoded, newline-delimited).
...
///
/// Internal builds (g6-releases) bake provider/model defaults and arbitrary
...
pub(crate) fn baked_build_env() -> BTreeMap<String, String> {
    build_env_map(
        option_env!("GEAR6_DESKTOP_BUILD_GEAR6_AGENT_PROVIDER"),
        option_env!("GEAR6_DESKTOP_BUILD_GEAR6_AGENT_MODEL"),
        option_env!("GEAR6_DESKTOP_BUILD_AGENT_ENV"),
    )
}
...
fn build_env_map(
    ...
    if let Some(provider) = raw_provider {
        if !provider.is_empty() {
            map.insert("GEAR6_AGENT_PROVIDER".to_string(), provider.to_string());
        }
    }
    if let Some(model) = raw_model {
        if !model.is_empty() {
            map.insert("GEAR6_AGENT_MODEL".to_string(), model.to_string());
        }
    }
...
pub(crate) fn build_g6_agent_provider_defaults(cmd: &mut std::process::Command) {
...
mod tests {
    use super::{
        baked_build_env, build_g6_agent_provider_defaults, build_env_map,
        discovery_env_with_baked_floor, parse_agent_env_lines,
    };

    #[test]
    fn g6_agent_provider_defaults_empty_in_oss_build() {
        // OSS (and normal test) builds set neither GEAR6_BUILD_GEAR6_AGENT_*,
        // so nothing is baked in and no GEAR6_AGENT_* is injected on spawn.
        let mut cmd = std::process::Command::new("env");
        cmd.env_clear();
        build_g6_agent_provider_defaults(&mut cmd);
        let output = cmd.output().expect("env should run");
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(
            !stdout.contains("GEAR6_AGENT_PROVIDER="),
            "GEAR6_AGENT_PROVIDER should not be injected in OSS builds"
        );
        assert!(
            !stdout.contains("GEAR6_AGENT_MODEL="),
            "GEAR6_AGENT_MODEL should not be injected in OSS builds"
        );
        assert!(
    ...
    //
    // `build_g6_agent_provider_defaults` must run BEFORE
    ...
    fn baked_defaults_do_not_override_record_provider_written_after() {
        ...
        cmd.env("GEAR6_AGENT_PROVIDER", "databricks");
        // Simulate what runtime_metadata_env_vars writes from the record (comes after).
        cmd.env("GEAR6_AGENT_PROVIDER", "anthropic");
        let output = cmd.output().expect("env should run");
        let stdout = String::from_utf8_lossy(&output.stdout);
        assert!(
            stdout.contains("GEAR6_AGENT_PROVIDER=anthropic"),
            "record provider must win over baked default (last-write-wins)"
        );
        assert!(
            !stdout.contains("GEAR6_AGENT_PROVIDER=databricks"),
    ...
    fn baked_build_env_is_empty_in_oss_build() {
        // In OSS/test builds none of the GEAR6_DESKTOP_BUILD_* compile-time vars
    ...
    fn build_env_map_provider_and_model_are_mapped() {
        ...
        assert_eq!(
            map.get("GEAR6_AGENT_PROVIDER").map(String::as_str),
            Some("databricks")
        );
        assert_eq!(
            map.get("GEAR6_AGENT_MODEL").map(String::as_str),
            Some("my-model")
        );
    }
    ...
    fn build_env_map_empty_provider_is_skipped() {
        ...
        assert!(
            !map.contains_key("GEAR6_AGENT_PROVIDER"),
