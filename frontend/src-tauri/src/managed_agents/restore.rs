...
pub fn backfill_persona_snapshots(app: &tauri::AppHandle) -> Result<(), String> {
    ...
    for record in records.iter_mut() {
        ...
        let Some(persona) = personas.iter().find(|p| p.id == persona_id) else {
            eprintln!(
                "g6-desktop: persona-snapshot backfill: agent {} links persona {persona_id} which no longer exists; leaving snapshot empty — it will spawn from its record fields",
                record.pubkey
            );
            continue;
        };
...
pub async fn restore_managed_agents_on_launch(
    ...
    let mut agents_to_start: Vec<super::ManagedAgentRecord>;
    {
        ...
        // Exact-path sweep: kill any g6-acp process whose executable path
        ...
        // GEAR6_MANAGED_AGENT injection or lost their PID-file receipt.
        //
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if let Err(e) =
                crate::commands::reconcile_agent_profile(&state, &reconcile_app, &pubkey, &data)
                    .await
            {
                eprintln!("g6-desktop: profile reconciliation failed for agent {pubkey}: {e}");
            }
        });
    }
