    ...
    match store.probe(&name) {
        ...
        KeyringProbe::Present | KeyringProbe::ReachableButEmpty => {
            match store.write_and_verify(&name, &record.private_key_nsec) {
                ...
                Err(e) => {
                    eprintln!(
                        "g6-desktop: keyring write for agent {} failed ({e}), keeping inline",
                        record.pubkey
                    );
...
/// `GEAR6_PRIVATE_KEY`/`NOSTR_PRIVATE_KEY`, launching with no identity. Callers
...
pub(crate) fn backup_invalid_store(path: &Path) {
    ...
    if let Err(e) = fs::copy(path, &backup) {
        eprintln!(
            "g6-desktop: failed to preserve malformed store {} as {}: {e}",
            path.display(),
            backup.display()
        );
    }
    ...
    for record in records.iter_mut() {
        ...
        if record.private_key_nsec.is_empty() {
            match store.load(&agent_keyring_name(&record.pubkey)) {
                ...
                Ok(None) => {
                    eprintln!(
                        "g6-desktop: agent {} has no key in JSON or keyring",
                        record.pubkey
                    );
                }
                ...
                Err(e) => {
                    eprintln!(
                        "g6-desktop: agent {} key unavailable — keyring read failed ({e}); \
...
/// (`"g6-desktop"`) to the dev service (`"g6-desktop-dev"`). Only runs
/// in debug builds — release builds never touch `"g6-desktop"` from this
/// path.
///
...
pub fn migrate_agent_keys_to_dev_service(app: &tauri::AppHandle) {
    if !cfg!(feature = "system-keyring") || keyring_service() != "g6-desktop-dev" {
        return;
    }
    ...
    let records = match load_agent_store(app) {
        Ok(r) => r,
        Err(e) => {
            eprintln!("g6-desktop: keyring-dev-migration: cannot read agent store: {e}");
            return;
        }
    };
    ...
    // singleton's cache.
    let prod_store = crate::secret_store::SecretStore::keyring("g6-desktop");
...
fn copy_agent_keys_between_stores(pubkeys: &[String], src: &impl KeyStore, dst: &impl KeyStore) {
    ...
    let dst_map: HashMap<String, String> = match dst.load_all_readonly() {
        ...
        Ok(None) => HashMap::new(),
        Err(e) => {
            eprintln!("g6-desktop: keyring-dev-migration: cannot read dev keyring: {e}");
            return;
        }
    };
        ...
        .all(|pubkey| dst_map.contains_key(&agent_keyring_name(pubkey)))
    {
        ...
        match src.load_all_readonly() {
            ...
            Err(e) => {
                eprintln!("g6-desktop: keyring-dev-migration: cannot read prod keyring: {e}");
                return;
            }
        }
    };
    ...
    if let Err(e) = dst.store_all(&to_write) {
        eprintln!("g6-desktop: keyring-dev-migration: cannot write to dev keyring: {e}");
        return;
    }

    if copied > 0 {
        eprintln!(
            "g6-desktop: keyring-dev-migration: copied {copied} agent key(s) from g6-desktop"
        );
    }
}
...
pub fn delete_agent_key(pubkey: &str) {
    if let Err(e) = try_delete_agent_key(pubkey) {
        eprintln!("g6-desktop: failed to delete agent {pubkey} key from keyring: {e}");
    }
}
...
pub fn meaningful_agent_error_from_log(path: &Path) -> Option<AgentLogError> {
    ...
    tail.lines().rev().map(str::trim).find_map(|line| {
        ...
        // Legacy format (older g6-acp builds): "Agent reported error: ..."
...
mod tests {
    ...
    fn spawn_refused_when_private_key_empty() {
        ...
        // before injecting an empty GEAR6_PRIVATE_KEY / NOSTR_PRIVATE_KEY — never
    ...
    fn strips_ansi_from_typical_tracing_line() {
        let input = "\x1b[2m2026-05-27T15:16:32\x1b[0m \x1b[32m INFO\x1b[0m \x1b[2mg6_acp\x1b[0m\x1b[2m:\x1b[0m starting";
        assert_eq!(
            strip_ansi_escapes::strip_str(input),
            "2026-05-27T15:16:32  INFO g6_acp: starting"
        );
    }
