//! Gear6 Nest — persistent agent workspace at `~/.g6`.
//!
//! Creates a shared knowledge directory on first launch so every
//! Gear6-spawned agent starts with orientation (AGENTS.md) and a
...
/// Default SKILL.md content for the g6-cli skill.
/// Written to ~/.g6/.agents/skills/g6-cli/SKILL.md on first init.
const GEAR6_CLI_SKILL_MD: &str = include_str!("nest_skill.md");
...
/// when the Tauri app-data directory name starts with `"xyz.block.g6.app.dev"`.
...
/// Returns `true` iff `path` ends with the dev-nest directory name (`.g6-dev`).
///
...
/// Returns `true` when the running binary is using the dev nest (`~/.g6-dev`).
///
...
/// directory starts with `"xyz.block.g6.app.dev"`.
///
/// Returns `false` when:
/// - The nest is the production nest (`~/.g6`, signed DMG).
...
/// Creates the Gear6 nest at `~/.g6` if it doesn't already exist.
///
...
/// Creates a Gear6 nest at the given `root` path.
///
/// - Creates the root directory and all subdirectories.
/// - Writes `AGENTS.md` only if it doesn't already exist.
/// - Writes `.agents/skills/g6-cli/SKILL.md` only if it doesn't already exist.
/// - Creates harness-specific symlinks pointing to the canonical
///   `.agents/skills/g6-cli` directory for each known provider.
...
pub fn ensure_nest_at(root: &Path) -> Result<(), String> {
    ...
    // Write g6-cli skill to the harness-agnostic .agents path.
        ...
        .open(&skill_md)
    {
        Ok(mut file) => {
            use std::io::Write;
            file.write_all(GEAR6_CLI_SKILL_MD.as_bytes())
    ...
    // Migration of the old .claude/skills/g6-cli real dir is handled in
...
///
/// Dev builds (`is_dev = true`) use `"g6-dev"` so that a running DMG and a
/// concurrent dev build each own a separate link and never clobber each other —
/// the same isolation that separates `~/.g6` (prod) from `~/.g6-dev` (dev).
...
/// Ensures `~/.local/bin/g6` (prod) or `~/.local/bin/g6-dev` (dev) is a
...
/// `~/.g6` and `~/.g6-dev` nests (see [`NEST_DIR_DEV`]).
///
/// On every boot: replaces any existing symlink unconditionally (the `g6` /
/// `g6-dev` name is our namespace), creates a new one if absent, and leaves
...
///
/// Preserves everything from the `<!-- BEGIN GEAR6 MANAGED` marker onward
...
fn refresh_skill_md_if_stale(root: &Path) -> Result<(), String> {
    let agents_skill_dir = root.join(".agents/skills/g6-cli");
    let version_path = agents_skill_dir.join(".skill-version");
    if read_version_file(&version_path) >= NEST_SKILL_VERSION {
        return Ok(());
    }

    // Migration: if .claude/skills/g6-cli exists as a real directory
    ...
    let skill_content = if old_is_real_dir {
        ...
        fs::read_to_string(old_skill_dir.join("SKILL.md"))
            .unwrap_or_else(|_| GEAR6_CLI_SKILL_MD.to_string())
    } else {
        GEAR6_CLI_SKILL_MD.to_string()
    };
    ...
    // Create/replace the .claude/skills/g6-cli symlink.
    #[cfg(unix)]
    {
        ...
        let symlink_path = root.join(".claude/skills/g6-cli");
        ...
        create_symlink(
            std::path::Path::new("../../.agents/skills/g6-cli"),
            &symlink_path,
        )
...
pub fn render_dynamic_section(
    ...
    let active_agents = if agents.is_empty() {
        "## Active Agents\n\n*(No agents deployed yet. Add agents in the Gear6 desktop app.)*"
            .to_string()
    } else {
        let mut table =
...
pub fn try_regenerate_nest(app: &AppHandle) {
    if let Err(error) = regenerate_nest_context(app) {
        eprintln!("g6-desktop: nest context regeneration failed: {error}");
    }
}
