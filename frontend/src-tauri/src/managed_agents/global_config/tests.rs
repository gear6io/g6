...
fn validate_rejects_reserved_key() {
    let config = config_with_env(&[("GEAR6_PRIVATE_KEY", "should-not-be-settable")]);
...
#[test]
fn validate_rejects_derived_key_g6_agent_provider() {
    let config = config_with_env(&[("GEAR6_AGENT_PROVIDER", "anthropic")]);
    let err = validate_global_config(&config).unwrap_err();
    assert!(
        err.contains("structured provider/model fields"),
        "got: {err}"
...
fn validate_ignores_empty_values_for_reserved_key_check() {
    ...
    let config = config_with_env(&[("GEAR6_PRIVATE_KEY", "")]);
...
fn inherited_shared_compute_translates_to_supported_agent_transport() {
    ...
    let runtime = super::super::known_acp_runtime("g6-agent").expect("g6-agent runtime");
