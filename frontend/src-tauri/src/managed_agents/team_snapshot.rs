//! `g6-team-snapshot v1` — manifest type, encoder, and decoder.
//!
...
//!   - `.team.png` — 1×1 placeholder PNG with manifest in a `g6_team_snapshot`
...
pub const PNG_CHUNK_KEYWORD: &str = "g6_team_snapshot";

/// Format discriminator — used for sniffing and validation.
pub const FORMAT_DISCRIMINATOR: &str = "g6-team-snapshot";
...
/// The top-level `g6-team-snapshot v1` manifest.
///
/// Serializes to / from JSON. Embedded in `.team.json` directly, or in the
/// `g6_team_snapshot` tEXt chunk of a `.team.png` (base64-encoded).
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
...
/// `g6_team_snapshot` tEXt chunk (base64-encoded JSON).
...
pub fn decode_team_snapshot_png(png_bytes: &[u8]) -> Result<TeamSnapshot, String> {
    ...
    let chunk_text = info
        ...
        .map(|c| c.text.as_str())
        .ok_or_else(|| "PNG does not contain a g6_team_snapshot tEXt chunk".to_string())?;
...
mod tests {
    ...
    fn format_and_version_correct_in_output() {
        let json = team_json_string();
        assert!(
            json.contains("\"g6-team-snapshot\""),
    ...
    fn decode_rejects_agent_snapshot_json_as_team_snapshot() {
        // An agent snapshot JSON (format: "g6-agent-snapshot") must NOT
