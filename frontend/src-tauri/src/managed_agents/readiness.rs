...
/// `effective_command` is the resolved harness binary name (e.g. `"g6-agent"`,
...
pub(crate) struct EffectiveAgentEnv {
    ...
    /// The resolved harness binary name (e.g. `"g6-agent"`, `"goose"`).
...
pub(crate) fn resolve_effective_agent_env(
    ...
    // Gear6 shared compute is a native Gear6 provider. Translate it to g6-agent's
...
pub enum Requirement {
    ...
    /// Git for Windows is missing, so g6-agent cannot launch g6-dev-mcp's
...
/// resolved runtime:
///
/// * **g6-agent / goose**: provider + model are required (both must be
...
/// `g6-agent/src/config.rs:143` — it is an escape hatch for static tokens
...
fn collect_missing_requirements(
    ...
    match rt.id {
        "g6-agent" => g6_agent_requirements(effective),
        "goose" => {
...
/// Requirements for g6-agent (provider + model + provider-specific creds).
fn g6_agent_requirements(effective: &EffectiveAgentEnv) -> Vec<Requirement> {
    ...
    // Provider is required — maps to GEAR6_AGENT_PROVIDER in the effective env.
    ...
    // Model is required — maps to GEAR6_AGENT_MODEL in the effective env.
    // Same empty-string treatment as provider.
    // Also accept provider-specific model fallback keys, matching g6-agent's
    ...
    // baked g6-releases env sets DATABRICKS_MODEL but not GEAR6_AGENT_MODEL,
    ...
    match provider {
        ...
        Some("databricks") | Some("databricks_v2") | Some("databricks-v2")
            ...
            // (OAuth PKCE is the normal path — see g6-agent/src/config.rs:143).
...
///
/// Mirrors g6-agent requirements but uses GOOSE_PROVIDER / GOOSE_MODEL.
///
...
/// require them from Gear6's env layer.  The file layer only *silences*
...
fn goose_requirements(
    ...
    // Empty string treated as absent — same as g6_agent_requirements.
    let provider = effective
        .env
        .get("GOOSE_PROVIDER")
        .filter(|v| !v.is_empty())
    ...
    // Provider-specific credentials — same empty-string semantics as g6-agent.
    let env_key_missing = |key: &str| effective.env.get(key).is_none_or(|v| v.is_empty());
...
mod tests {
    ...
    #[test]
    fn g6_agent_missing_provider_returns_not_ready_with_normalized_field() {
        let env = make_env(
            "g6-agent",
            env_with(&[("GEAR6_AGENT_MODEL", "claude-opus-4-5")]),
        );
        let result = agent_readiness(&env);
        assert!(
            !result.is_ready(),
            "missing GEAR6_AGENT_PROVIDER should be NotReady"
        );
    ...
    #[test]
    fn g6_agent_missing_model_returns_not_ready_with_normalized_field() {
        let env = make_env(
            "g6-agent",
            env_with(&[
                ("GEAR6_AGENT_PROVIDER", "anthropic"),
    ...
    #[test]
    fn g6_agent_missing_anthropic_key_returns_not_ready_with_env_key() {
        let env = make_env(
            "g6-agent",
            env_with(&[
                ("GEAR6_AGENT_PROVIDER", "anthropic"),
                ("GEAR6_AGENT_MODEL", "claude-opus-4-5"),
            ]),
        );
        let result = agent_readiness(&env);
        assert!(!result.is_ready());
    ...
    #[test]
    fn g6_agent_missing_openai_key_returns_not_ready() {
        let env = make_env(
            "g6-agent",
            env_with(&[
                ("GEAR6_AGENT_PROVIDER", "openai"),
                ("GEAR6_AGENT_MODEL", "gpt-4o"),
            ]),
        );
        let result = agent_readiness(&env);
        assert!(!result.is_ready());
    ...
    #[test]
    fn g6_agent_anthropic_with_all_fields_is_ready() {
        let env = make_env(
            "g6-agent",
            env_with(&[
                ("GEAR6_AGENT_PROVIDER", "anthropic"),
    ...
    #[test]
    fn g6_agent_databricks_with_host_and_model_is_ready_without_token() {
        ...
        let env = make_env(
            "g6-agent",
            env_with(&[
                ("GEAR6_AGENT_PROVIDER", "databricks"),
                ("GEAR6_AGENT_MODEL", "dbrx-instruct"),
    ...
    #[test]
    fn g6_agent_databricks_missing_host_returns_not_ready() {
        let env = make_env(
            "g6-agent",
            env_with(&[
                ("GEAR6_AGENT_PROVIDER", "databricks"),
    ...
    #[test]
    fn g6_agent_databricks_v2_missing_host_returns_not_ready() {
        let env = make_env(
            "g6-agent",
            env_with(&[
    ...
    #[test]
    fn g6_agent_empty_string_provider_is_not_ready() {
        let env = make_env(
            "g6-agent",
            env_with(&[
        ...
        assert!(
            !result.is_ready(),
            "empty-string GEAR6_AGENT_PROVIDER must be treated as missing"
        );
        assert!(result
            .requirements()
            .contains(&Requirement::NormalizedField {
    ...
    #[test]
    fn g6_agent_empty_string_model_is_not_ready() {
        let env = make_env(
            "g6-agent",
            env_with(&[
                ("GEAR6_AGENT_PROVIDER", "anthropic"),
        ...
        assert!(
            !result.is_ready(),
            "empty-string GEAR6_AGENT_MODEL must be treated as missing"
        );
        assert!(result
            .requirements()
            .contains(&Requirement::NormalizedField {
    ...
    #[test]
    fn g6_agent_empty_string_anthropic_key_is_not_ready() {
        let env = make_env(
            "g6-agent",
            env_with(&[
                ("GEAR6_AGENT_PROVIDER", "anthropic"),
    ...
    #[test]
    fn g6_agent_empty_string_databricks_host_is_not_ready() {
        let env = make_env(
            "g6-agent",
            env_with(&[
                ("GEAR6_AGENT_PROVIDER", "databricks"),
    ...
    fn codex_not_ready_copy_does_not_mention_openai_api_key() {
        ...
        let rt = make_cli_runtime(&["__g6_nonexistent_adapter_xyz789__"], None);
    ...
    fn cli_login_requirements_missing_binary_is_not_ready() {
        ...
        let rt = make_cli_runtime(
            &["__g6_nonexistent_adapter_abc123__"],
            Some("__g6_nonexistent_cli_abc123__"),
        );
        let reqs = cli_login::requirements(
            &["__g6_nonexistent_binary_abc123__", "status"],
    ...
    fn cli_login_requirements_adapter_missing_emits_adapter_missing() {
        ...
        let rt = make_cli_runtime(&["__g6_nonexistent_adapter_xyz789__"], Some(exe));
    ...
    fn cli_login_requirements_cli_missing_emits_cli_missing() {
        ...
        let rt = make_cli_runtime(
            ...
            Some("__g6_nonexistent_cli_abc123__"), // underlying CLI missing
        );
    ...
    fn cli_login_requirements_logged_out_emits_available() {
        ...
        let rt = make_cli_runtime(static_commands(vec![exe]), Some(exe));
        let reqs =
            cli_login::requirements(&[exe, "--g6-probe-fail-xyz"], "run `tool login`", &rt);
        assert!(
            !reqs.is_empty(),
    ...
    fn cli_login_requirements_codex_outdated_adapter_emits_adapter_outdated() {
        ...
        // intentionally prefers Gear6's managed npm shim when it exists, which
    ...
    fn resolve_effective_agent_env_user_env_wins_over_structured_fields() {
        ...
        env_vars.insert("GEAR6_AGENT_PROVIDER".to_string(), "anthropic".to_string());
        env_vars.insert(
            "GEAR6_AGENT_MODEL".to_string(),
        ...
        let runtime = known_acp_runtime_exact("g6-agent");
        ...
        assert_eq!(
            effective.env.get("GEAR6_AGENT_PROVIDER").map(String::as_str),
            Some("anthropic")
        );
        assert_eq!(
            effective.env.get("GEAR6_AGENT_MODEL").map(String::as_str),
    ...
    #[test]
    fn g6_agent_databricks_v2_with_databricks_model_but_no_g6_agent_model_is_ready() {
        // The baked g6-releases env sets DATABRICKS_MODEL but not GEAR6_AGENT_MODEL.
    ...
    #[test]
    fn g6_agent_databricks_v2_hyphen_alias_with_databricks_model_is_ready() {
        // g6-agent accepts both "databricks_v2" and "databricks-v2". The
        // readiness gate must recognize the hyphen alias and accept DATABRICKS_MODEL.
        let env = make_env(
            "g6-agent",
            env_with(&[
                ("GEAR6_AGENT_PROVIDER", "databricks-v2"),
                ("DATABRICKS_MODEL", "goose-claude-4-6-sonnet"),
                ("DATABRICKS_HOST", "https://dbc.example.com"),
            ]),
        );
    ...
    #[test]
    fn g6_agent_databricks_hyphen_alias_missing_host_returns_not_ready() {
    ...
    #[test]
    fn g6_agent_databricks_v1_with_databricks_model_but_no_g6_agent_model_is_ready() {
    ...
    #[test]
    fn g6_agent_anthropic_with_anthropic_model_but_no_g6_agent_model_is_ready() {
        let env = make_env(
            "g6-agent",
            env_with(&[
                ("GEAR6_AGENT_PROVIDER", "anthropic"),
    ...
    #[test]
    fn g6_agent_openai_with_openai_compat_model_but_no_g6_agent_model_is_ready() {
        let env = make_env(
            "g6-agent",
            env_with(&[
                ("GEAR6_AGENT_PROVIDER", "openai"),
    ...
    #[test]
    fn g6_agent_empty_provider_model_fallback_key_is_not_ready() {
        // An empty DATABRICKS_MODEL with no GEAR6_AGENT_MODEL must still be NotReady.
        let env = make_env(
            "g6-agent",
            env_with(&[
                ("GEAR6_AGENT_PROVIDER", "databricks_v2"),
        ...
        assert!(
            !result.is_ready(),
            "empty DATABRICKS_MODEL with no GEAR6_AGENT_MODEL must be NotReady"
        );
        assert!(result
            .requirements()
            .contains(&Requirement::NormalizedField {
