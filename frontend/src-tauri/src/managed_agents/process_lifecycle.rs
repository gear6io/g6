...
/// child (g6-acp) does spawn its 24 workers before it connects to the relay,
...
/// are a few synchronous Win32 calls (microseconds), while g6-acp must init
...
pub fn finish_spawn(
    ...
    if job.is_none() {
        eprintln!(
            "g6-desktop: failed to assign agent {agent_name} to a Job Object; \
