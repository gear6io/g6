//! `g6-agent-snapshot v1` — manifest type, encoder, and decoder stubs.
//!
...
//!   - `.agent.png` — avatar image with manifest in a `g6_agent_snapshot`
//!     tEXt chunk
//!
...
pub const PNG_CHUNK_KEYWORD: &str = "g6_agent_snapshot";
...
pub const FORMAT_DISCRIMINATOR: &str = "g6-agent-snapshot";
...
/// The top-level `g6-agent-snapshot v1` manifest.
///
/// Serializes to / from JSON. Embedded in `.agent.json` directly, or in the
/// `g6_agent_snapshot` tEXt chunk of a `.agent.png` (base64-encoded).
#[derive(Debug, Clone, Serialize, Deserialize, PartialEq)]
#[serde(rename_all = "camelCase")]
...
/// in the `g6_agent_snapshot` tEXt chunk.
...
pub fn decode_snapshot_png(png_bytes: &[u8]) -> Result<AgentSnapshot, String> {
    ...
    let chunk_text = info
        ...
        .map(|c| c.text.as_str())
        .ok_or_else(|| "PNG does not contain a g6_agent_snapshot tEXt chunk".to_string())?;
...
mod tests {
    ...
    fn invalid_format_discriminator_is_rejected() {
        ...
        snapshot.format = "not-a-g6-snapshot".to_string();
