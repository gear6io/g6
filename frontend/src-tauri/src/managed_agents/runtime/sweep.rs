...
//! or that predates `GEAR6_MANAGED_AGENT` injection. This sweep derives the
//! expected `g6-acp` path from the running executable and kills any process
...
///    intermediates run in their own process groups (e.g. g6-acp ->
///    node shim -> codex-acp).
#[cfg(target_os = "macos")]
...
///
/// On Linux, `read_link("/proc/<pid>/exe")` returns `…/g6-acp (deleted)`
...
/// `extract_g6_marker_value` / `process_has_g6_marker` which must skip
...
/// binary name (`g6-acp`) proceed to the full exe-path fetch.  This mirrors
...
/// processes but at most a handful of `g6-acp` instances).
#[cfg(target_os = "macos")]
...
fn collect_process_snapshots(harness_name: &str) -> Vec<ProcessSnapshot> {
    ...
    for entry in entries.flatten() {
        ...
        // "g6-acp" is 8 chars so it's always preserved).
...
/// Derive the expected path of the `g6-acp` harness binary next to the
/// current executable. Returns `None` if `current_exe()` fails or has no
/// parent directory.
///
/// In a `.app` bundle: `.../Contents/MacOS/g6-acp`.
/// In a dev checkout: `<target-dir>/debug/g6-acp` or similar.
...
/// scopes by `GEAR6_MANAGED_AGENT` instance ID rather than exe path.
pub fn expected_harness_exe_path() -> Option<PathBuf> {
    let exe = std::env::current_exe().ok()?;
    let dir = exe.parent()?;
    let raw = dir.join("g6-acp");
...
const HARNESS_BINARY_NAME: &str = "g6-acp";

// ── sweep_untracked_bundle_harnesses ─────────────────────────────────────

/// Sweep and kill harness processes that share this bundle's exact `g6-acp`
/// executable path but are not in `skip_pids`.
///
/// Complements the env-var-based `sweep_system_agent_processes`: this sweep
/// catches orphans that predate the `GEAR6_MANAGED_AGENT` env var injection
...
/// where possible) equals `<this bundle>/g6-acp` are candidates. Dev builds
...
pub(crate) fn sweep_untracked_bundle_harnesses(skip_pids: &[u32]) {
    ...
    eprintln!(
        "g6-desktop: sweep_untracked_bundle_harnesses: reaping {} stale harness process(es) {:?} (exe: {})",
        to_kill.len(),
        to_kill,
        harness_exe.display(),
    );
...
mod tests {
    ...
    fn strip_deleted_suffix_removes_kernel_suffix() {
        ...
        let p = PathBuf::from("/Applications/Gear6.app/Contents/MacOS/g6-acp (deleted)");
        assert_eq!(
            strip_deleted_suffix(p),
            PathBuf::from("/Applications/Gear6.app/Contents/MacOS/g6-acp")
        );
    }

    #[test]
    fn strip_deleted_suffix_leaves_normal_path_unchanged() {
        let p = PathBuf::from("/Applications/Gear6.app/Contents/MacOS/g6-acp");
    ...
    fn strip_deleted_suffix_does_not_strip_partial_match() {
        ...
        let p = PathBuf::from("/some/path/g6-acp(deleted)");
        assert_eq!(strip_deleted_suffix(p.clone()), p,);
    }

    // ── select_untracked_bundle_harnesses ────────────────────────────────

    const BUNDLE_HARNESS: &str = "/Applications/Gear6.app/Contents/MacOS/g6-acp";
    const DEV_HARNESS: &str = "/Users/dev/g6/.worktrees/main/target/debug/g6-acp";
    ...
    fn child_of_tracked_parent_not_directly_targeted() {
        ...
        let snapshots = vec![snap(1004, "/Applications/Gear6.app/Contents/MacOS/goose")];
        let result =
            select_untracked_bundle_harnesses(&snapshots, &PathBuf::from(BUNDLE_HARNESS), &[]);
        assert!(result.is_empty());
    }
    ...
    fn deleted_suffix_stripped_path_matches_expected() {
        ...
        let raw = PathBuf::from("/Applications/Gear6.app/Contents/MacOS/g6-acp (deleted)");
