...
use super::{
    ...
    process_has_g6_marker, process_is_running, terminate_process, ManagedAgentPairRuntime,
...
fn stop_managed_agent_pair(
    ...
    let result = (|| -> Result<(), String> {
        ...
        if let Err(error) = append_log_marker(
            ...
            eprintln!(
                "g6-desktop: failed to append stop marker for {} on {}: {error}",
...
fn stop_legacy_scalar_pid(app: &AppHandle, record: &mut ManagedAgentRecord) -> Result<(), String> {
    if let Some(pid) = record.runtime_pid.take() {
        if process_is_running(pid)
            ...
            && process_has_g6_marker(pid, &current_instance_id(app))
        {
