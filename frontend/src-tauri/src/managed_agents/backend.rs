...
/// Enumerate PATH for g6-backend-* executables. Returns (id, path) pairs.
...
/// `resolve_command(format!("g6-backend-{id}"))` to prevent a compromised
...
pub fn resolve_provider_binary(provider_id: &str) -> Result<PathBuf, String> {
    ...
    match found {
        ...
        None => Err(format!(
            "provider 'g6-backend-{provider_id}' not found on PATH"
        )),
    }
}
