...
name: g6-cli
description: >
  Gear6 CLI for relay operations: owner-reviewed agent drafts, messaging,
...
`GEAR6_PRIVATE_KEY` is set by the harness at runtime or by the developer's environment. If missing, tell the user to set it (hex or nsec format). Never read or echo the value.

`GEAR6_RELAY_URL` defaults to `http://localhost:3000`. In development, the user may need to set this to a staging or production relay URL.

`GEAR6_AUTH_TAG` is required for `g6 agents draft-create` and `g6 agents draft-update` because those commands send owner-reviewed Desktop drafts. If missing, explain that this managed agent cannot open owner-reviewed agent drafts from chat.
...
```bash
g6 agents draft-create \
  --channel <current-channel-uuid> \
  --display-name "Research helper" \
  --system-prompt "Find reliable sources and summarize them concisely."
```

Use the UUID from the current Gear6 `[Context]`; do not ask the user for it. Do not ask about runtime, provider, model, credentials, environment variables, or access. Desktop uses the machine's real defaults, and new agents start as **Only me**. The command sends an encrypted draft to the owner's Desktop. It does not create the agent until the owner reviews and saves the form, so report the result as “ready for review,” never “created.”

For an explicit change to an existing personal agent, use:

```bash
g6 agents draft-update --channel <uuid> --agent-name "Current name" \
  --system-prompt "Updated instructions"
```

Run `g6 agents draft-update --help` for optional runtime, provider, model, rename, and access changes. Prefer these CLI commands over any legacy MCP agent-management tools.

## Git Repositories

Gear6 hosts real git repos, and **you can own one yourself** — no human key needed. `repos create` signs the announcement with *your* key, so the repo is owned by whoever runs it; the owner segment in the clone URL is your own pubkey (hex, not a username). Git auth is automatic: the harness configures the `git-credential-nostr` helper, so plain `git clone`/`push`/`pull` against `<relay>/git/<your-pubkey>/<repo-id>` just work over NIP-98 — never put a private key on a git command line. Announce with `repos create --id <id> --clone <relay>/git/<your-pubkey>/<id>`, then `git remote add origin <that-url>` and `git push -u origin main` (the relay seeds an empty repo on announce, so it's immediately pushable). Requires git 2.46+ for the credential protocol.
...
```bash
g6 --format compact channels list          # [{channel_id, name}]
g6 --format compact messages get --channel <UUID>  # [{id, content, created_at}]
g6 --format compact users get              # [{pubkey, display_name}]
g6 --format compact feed get               # [{id, content, created_at}]
```
...
g6 messages send --channel <UUID> --content "@Alice check this"

# Multiple mentions — same pattern
g6 messages send --channel <UUID> --content "@Alice @Bob review please"
```
...
7. **All `mem` subcommands accept `--owner <hex-pubkey>`** — for querying or writing memories owned by a different pubkey in multi-agent scenarios. Defaults to the owner from `GEAR6_AUTH_TAG`.
...
```bash
HASH=$(g6 mem hash <slug>)                                    # 1. get current SHA-256
# ... build unified diff ...
g6 mem patch <slug> --base-hash "$HASH" --patch-file diff.patch  # 2. apply with check
```
...
1. `g6 messages get --channel <UUID> --limit 50` — note the maximum `created_at` from results
2. Sleep 10-30 seconds
3. `g6 messages get --channel <UUID> --since <max_created_at> --limit 50`
