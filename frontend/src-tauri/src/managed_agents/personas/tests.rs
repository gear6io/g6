...
fn fizz_builtin_has_no_pinned_runtime() {
    ...
    // bundled default (g6-agent) rather than requiring goose on PATH.
...
#[test]
fn fizz_builtin_resolves_to_g6_agent() {
    // With no runtime pin, effective_agent_command must fall through to
    // default_agent_command(), which resolves the bundled g6-agent.
    let records = built_in_persona_records("2026-01-01T00:00:00Z");
    assert_eq!(
    ...
    assert_eq!(
        effective_agent_command(Some("builtin:fizz"), &records, None),
        "g6-agent",
        "Fizz must resolve to g6-agent specifically"
    );
}
