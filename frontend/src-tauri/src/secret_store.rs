...
//!
//! The store is deliberately NOT on any env-read path. `GEAR6_PRIVATE_KEY`
...
//
// Two concurrent Gear6 processes (e.g. the signed DMG build and an unsigned dev
// build via `just staging`) share the same OS keychain blob because the
// service name `"g6-desktop"` is a constant — it does not key off the bundle
...
// per-user path `/tmp/g6-keychain-<uid>-<service>.lock` on Unix — a path
...
///
/// The path is `/tmp/g6-keychain-<uid>-<service>.lock` on Unix — a
...
impl SecretStore {
    ...
    ///
    /// Only one service name (`"g6-desktop"`) is used in practice. If a
...
impl SecretStore {
    ...
    {
        // Acquire the interprocess advisory lock first. All Gear6 processes
        // using the same service name contend on the same lockfile at
        // /tmp/g6-keychain-<uid>-<service>.lock (a deterministic per-user
...
mod tests {
    ...
    fn probe_returns_present_when_key_in_cache() {
        ...
        let store = SecretStore::with_cache("g6-test-cache-hit", Some(map));
    ...
    fn load_returns_value_when_key_in_cache() {
        let mut map = HashMap::new();
        map.insert("identity".to_string(), "nsec1test".to_string());
        let store = SecretStore::with_cache("g6-test-load-cache-hit", Some(map));
    ...
    fn test_stale_warm_cache_add_observes_prior_write() {
        ...
        let svc = "g6-test-race-stale-cache";
    ...
    fn test_concurrent_adds_neither_key_dropped() {
        ...
        let svc = "g6-test-race-concurrent-add";
    ...
    fn test_blob_lockfile_path_is_in_tmp_with_uid() {
        ...
        let path = blob_lockfile_path("g6-desktop");
        #[cfg(unix)]
        {
            let uid = unsafe { libc::getuid() };
            assert!(
            ...
            assert!(
                name.contains("g6-keychain"),
                "lockfile name must contain 'g6-keychain'"
            );
        }
        #[cfg(not(unix))]
        {
            assert!(
                path.file_name()
                    .and_then(|n| n.to_str())
                    .is_some_and(|n| n.contains("g6-keychain")),
                "lockfile name must contain 'g6-keychain'"
            );
        }
    }
    ...
    fn test_blob_lock_acquire_and_release() {
        ...
        let guard = acquire_blob_lock("g6-test-lock-smoke");
        assert!(
            guard.is_ok(),
        ...
        drop(guard);
        let guard2 = acquire_blob_lock("g6-test-lock-smoke");
        assert!(
            guard2.is_ok(),
    ...
    fn mutate_blob_does_not_advance_cache_on_write_failure() {
        ...
        //   cargo test -p g6-desktop -- --ignored mutate_blob_does_not_advance
        //
        ...
        let store = SecretStore::with_cache("g6-test-cow-write-fail", Some(map));
    ...
    //   cargo test -p g6-desktop -- --ignored blob_
    //
    ...
    fn blob_stores_and_retrieves_multiple_keys() {
        let store = SecretStore::keyring("g6-test-blob-multi");
    ...
    fn blob_probe_present_absent_unreachable() {
        let store = SecretStore::keyring("g6-test-blob-probe");
    ...
    fn blob_delete_removes_key_not_others() {
        let store = SecretStore::keyring("g6-test-blob-delete");
    ...
    fn blob_migration_from_per_key_entry() {
        let svc = "g6-test-blob-migration";
        let key = "identity";
    ...
    fn delete_all_with_legacy_cleanup_removes_per_key_identity() {
        let svc = "g6-test-delete-all-legacy";
        let key = "identity";
