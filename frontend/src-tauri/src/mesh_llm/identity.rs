...
//! Gear6/Nostr identity). The node presents a signed ownership attestation
...
//! has one owner identity whether mesh runs embedded in Gear6 or standalone.
...
impl OwnerIdentity {
    /// Sign a Gear6-to-MeshLLM ownership binding. The member's Nostr signature
...
pub fn member_endpoint_binding_bytes(member_pubkey: &str, endpoint_tokens: &[String]) -> Vec<u8> {
    ...
    format!(
        "g6-mesh-owner-endpoint-binding-v1:{}:{}",
        member_pubkey.trim().to_ascii_lowercase(),
