...
/// Gear6-curated tier picks. These are the models we know survive the agent
...
const CURATED_LARGE_DESCRIPTION: &str =
    "Gemma 4 26B MoE (4B active) — Gear6 default for 64GB+ machines";
...
/// The Gear6-curated recommendation for a machine's rated memory capacity.
fn g6_recommended_model(rated_gb: Option<u64>) -> &'static str {
    match rated_gb {
...
pub struct MeshCatalogEntry {
    ...
    /// Gear6-curated pick — known to survive the agent harness. Curated
...
fn build_catalog(
    ...
    // The compiled MODEL_CATALOG does not know the Gear6 large pick; it
    ...
    let recommended = Some(g6_recommended_model(rated_capacity_gb(vram_bytes)).to_string());
...
mod tests {
    ...
    #[test]
    fn recommendation_follows_g6_curated_tiers() {
