...
/// (e.g. `xyz.block.g6.app` or `xyz.block.g6.app.dev`).
...
pub(crate) struct ResetContext<'a> {
    ...
    /// Nest dir (`~/.g6` or `~/.g6-dev`) scoped to this build's variant,
...
pub(crate) fn run_boot_reset_with_keychain(ctx: ResetContext<'_>) -> ResetOutcome {
    ...
    // ── Step 3: remove nest, ~/.sprout, ~/.config/g6-agent, CLI symlink ────
    ...
    if let Some(ref home) = ctx.home_dir {
        ...
        let _ = std::fs::remove_dir_all(home.join(".config").join("g6-agent"));
    ...
    if let Err(e) = ctx.keychain.delete_all_with_legacy() {
        eprintln!("g6-desktop reset: keychain delete: {e}");
    ...
    {
        eprintln!(
            "g6-desktop reset: verification failed (keychain_wiped={keychain_ok}, \
    ...
    if let Err(e) = delete_sentinel(app_data_dir) {
        eprintln!("g6-desktop reset: delete sentinel: {e}");
...
mod tests {
    ...
    fn test_dev_build_wipes_dev_nest_not_prod() {
        ...
        // Create both nests.
        let dev_nest = tmp.path().join(".g6-dev");
        let prod_nest = tmp.path().join(".g6");
    ...
    fn test_completed_dev_reset_suppresses_dev_repos_dir_migration() {
        ...
        // Seed prod ~/.g6/.repos-dir so the migration has something to copy.
        let home = tmp.path().join("home");
        let prod_nest = home.join(".g6");
        std::fs::create_dir_all(&prod_nest).unwrap();
        ...
        let dev_nest_2 = tmp.path().join(".g6-dev-control");
        ...
        let dev_nest_3 = tmp.path().join(".g6-dev-prod");
    ...
    fn test_crash_retry_cleans_prior_deterministic_trash() {
        ...
        let app_data = app_support.join("xyz.block.g6.app");
        std::fs::create_dir_all(&app_data).unwrap();
        write_sentinel(&app_data).unwrap();
        ...
        let trash_app_dir = app_support.join("xyz.block.g6.app.reset-trash");
    ...
    fn test_keychain_fail_restores_all_then_retry_cleans() {
        ...
        let trash_app = app_support.join("xyz.block.g6.app.reset-trash");
