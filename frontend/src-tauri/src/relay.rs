...
// nostr 0.36 alias — required for cross-version bridging with g6-sdk.
...
pub fn relay_ws_url() -> String {
    configured_env_var("GEAR6_RELAY_URL")
        .or_else(|| option_env!("GEAR6_DESKTOP_BUILD_RELAY_URL").map(str::to_string))
...
pub fn relay_api_base_url() -> String {
    if let Some(base) = configured_env_var("GEAR6_RELAY_HTTP") {
        return base.trim_end_matches('/').to_string();
    }

    if let Some(base) = option_env!("GEAR6_DESKTOP_BUILD_RELAY_HTTP") {
...
///
/// Gear6's command kinds (e.g. 41010, 30620, 46020) acknowledge writes via
...
///
/// `g6-sdk` uses `nostr 0.36` while the desktop crate uses `nostr 0.37`. Cross-version
...
fn build_profile_event(
    ...
    let builder = if let Some(tag_json) = auth_tag_json {
        ...
        g6_sdk_pkg::nip_oa::verify_auth_tag(tag_json, &compat_pubkey)
        ...
        let compat_tag = g6_sdk_pkg::nip_oa::parse_auth_tag(tag_json)
...
mod tests {
    ...
    /// `g6_sdk_pkg::nip_oa::compute_auth_tag` expects nostr 0.36 types.
    ...
    fn make_valid_auth_tag(agent_keys: &nostr::Keys) -> String {
        ...
        g6_sdk_pkg::nip_oa::compute_auth_tag(&owner_keys, &agent_compat_pubkey, "")
