...
pub struct AcpAuthMethod {
    ...
    #[serde(default)]
    pub args: Vec<String>,
    /// Full terminal command advertised by the adapter. Gear6 never guesses
...
fn discover_acp_auth_methods_blocking(runtime_id: &str) -> Result<AcpAuthMethodsResult, String> {
    let output = run_g6_acp_auth_command(runtime_id, ["auth-methods", "--json"])?;
    if !output.status.success() {
        return Err(command_error("g6-acp auth-methods", &output));
    }
...
fn connect_acp_runtime_blocking(
    ...
    let output = run_g6_acp_auth_command(
        &request.runtime_id,
        ["authenticate", "--method-id", request.method_id.as_str()],
    )?;
    if !output.status.success() {
        return Err(command_error("g6-acp authenticate", &output));
    }

    Ok(ConnectAcpRuntimeResult { launched: true })
}

fn run_g6_acp_auth_command<const N: usize>(
    runtime_id: &str,
    args: [&str; N],
    ...
    let acp_path = std::env::current_exe()
        .map(|path| path.with_file_name(format!("g6-acp{}", std::env::consts::EXE_SUFFIX)))
        .ok()
        .filter(|path| path.exists())
        .or_else(|| resolve_command("g6-acp"))
        .ok_or_else(|| "g6-acp helper not found".to_string())?;

    let augmented_path = auth_command_path();
    run_g6_acp_auth_command_with_paths(
        &acp_path,
        adapter_command.0,
        &adapter_command.1,
        args,
...
/// PATH for the g6-acp auth helper child process.
///
/// Uses the augmented agent PATH so `#!/usr/bin/env node` adapter shims
/// resolve the Gear6-managed Node runtime — the same PATH normal agent
/// launches and readiness probes use.
///
/// On Windows, `login_shell_path()` is intentionally `None`, so the augmented
/// PATH contains only Gear6-managed directories and the exe parent. Gear6 does
...
fn run_g6_acp_auth_command_with_paths<const N: usize>(
    acp_path: &Path,
    adapter_name: &str,
    adapter_path: &Path,
    args: [&str; N],
    ...
    command
        .args(args)
        .env("GEAR6_ACP_AGENT_COMMAND", adapter_path.as_os_str())
        .env("GEAR6_ACP_AGENT_ARGS", agent_args.join(","))
        .stdout(Stdio::piped())
        .stderr(Stdio::piped());
    ...
    command
        .output()
        .map_err(|error| format!("failed to run g6-acp auth helper: {error}"))
}
...
mod tests {
    use super::{
        ...
        run_g6_acp_auth_command_with_paths, shell_escape, shell_join, uses_terminal_auth,
        windows_terminal_args, AcpAuthMethod,
    };

    /// Windows regression: the augmented PATH there holds only Gear6-managed
    ...
    fn append_inherited_path_appends_after_augmented() {
        ...
        let augmented = format!("{0}g6-bin{1}{0}exe-dir", std::path::MAIN_SEPARATOR, sep);
    ...
    fn auth_command_uses_augmented_path_for_node_adapter() {
        ...
        let acp_path = temp.path().join("g6-acp");
        fs::write(&acp_path, "#!/bin/sh\nexec \"$GEAR6_ACP_AGENT_COMMAND\"\n")
            .expect("write g6-acp");
        fs::set_permissions(&acp_path, fs::Permissions::from_mode(0o755)).expect("chmod g6-acp");

        let augmented_path = std::env::join_paths([interpreter_dir.as_path()])
            .expect("join augmented PATH")
            .to_string_lossy()
            .into_owned();
        let output = run_g6_acp_auth_command_with_paths(
            &acp_path,
            "claude-agent-acp",
            &adapter_path,
            ["auth-methods", "--json"],
...
"#
    ...
    fn terminal_argv_uses_adapter_declared_command() {
        ...
        let method = AcpAuthMethod {
            ...
            command: vec![
                "definitely-not-on-path-g6-test".into(),
                "auth".into(),
                "login".into(),
            ],
            meta: None,
        ...
        assert_eq!(
            ...
            vec![
                "definitely-not-on-path-g6-test".to_string(),
                "auth".to_string(),
                "login".to_string()
            ]
        );
    ...
    fn terminal_argv_falls_back_to_adapter_command() {
        ...
        assert_eq!(
            adapter_terminal_argv("Claude Code", &method, "definitely-not-on-path-g6-test")
                .unwrap(),
            vec!["definitely-not-on-path-g6-test".to_string()]
        );
    }
}
