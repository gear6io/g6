    ...
    tokio::task::spawn_blocking(move || {
        ...
        if let Some(nest) = nest.as_deref() {
            if let Err(error) = write_persisted_repos_dir(nest, effective_repos_dir.as_deref()) {
                eprintln!("g6-desktop: persist repos dir failed: {error}");
            }
            if let Err(error) = ensure_repos_symlink(nest, effective_repos_dir.as_deref()) {
                eprintln!("g6-desktop: repos dir setup failed: {error}");
                let _ = app.emit("repos-dir-error", error);
            }
        }
    ...
    #[cfg(feature = "mesh-llm")]
    {
        ...
        tauri::async_runtime::spawn(async move {
            ...
            if restore_pending {
                ...
                {
                    eprintln!("g6-desktop: failed to restore Share Compute: {error}");
                }
            }
            ...
            if restore_pending {
                ...
                {
                    eprintln!("g6-desktop: failed to restore managed agents: {error}");
                }
            }
        });
    }
