//! Tauri commands for exporting and importing `g6-team-snapshot v1` files.
//!
...
const LEGACY_TEAM_ERROR: &str =
    "Legacy team files are no longer supported. Export a g6-team-snapshot v1 .team.json or .team.png instead.";
    ...
    for (m, snap_member) in minted.iter().zip(snapshot.members.iter()) {
        ...
        if memory_total > 0 {
            ...
            for (idx, entry) in snap_member.memory.entries.iter().enumerate() {
                ...
                match g6_core_pkg::engram::build_event(
                    &m.agent_keys,
                    &owner_pubkey,
                    &body,
                    created_at,
...
fn retain_agent_pending(app: &AppHandle, state: &AppState, record: &ManagedAgentRecord) {
    ...
    if let Err(e) = result {
        eprintln!("g6-desktop: team-snapshot-import retain-agent: {e}");
    }
}
