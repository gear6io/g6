...
pub(crate) fn sign_blossom_get_auth_header(
    ...
    let event = EventBuilder::new(Kind::from(24242), "Get g6-media")
        .tags(tags)
        .sign_with_keys(keys)
        .map_err(|e| e.to_string())?;
    Ok(format!(
...
///
/// Fail-open by design: while the relay's `GEAR6_REQUIRE_MEDIA_GET_AUTH` flag
...
pub(crate) fn mint_media_get_auth(state: &AppState, base_url: &str) -> Option<String> {
    let keys = match state.signing_keys() {
        Ok(k) => k,
        Err(e) => {
            eprintln!("g6-desktop: media get auth unavailable (unsigned request): {e}");
            return None;
        }
    };
    match sign_blossom_get_auth_header(&keys, base_url, MEDIA_GET_AUTH_EXPIRY_SECS) {
        ...
        Err(e) => {
            eprintln!("g6-desktop: media get auth signing failed (unsigned request): {e}");
            None
        }
    }
}
...
fn sign_blossom_upload_auth(
    ...
    EventBuilder::new(Kind::from(24242), "Upload g6-media")
        .tags(tags)
        .sign_with_keys(keys)
        .map_err(|e| e.to_string())
}
...
async fn process_picked_path(
    ...
    if let Some(poster) = poster_bytes {
        match do_upload(poster, "image/jpeg", state, None).await {
            ...
            Err(e) => eprintln!("g6-desktop: poster upload failed (non-fatal): {e}"),
        }
    }
...
pub async fn upload_media_bytes(
    ...
    let (body, poster_bytes) = if is_video_file(&data) {
        ...
        tokio::task::spawn_blocking(move || -> Result<(Vec<u8>, Option<Vec<u8>>), String> {
            let tmp_input =
                std::env::temp_dir().join(format!("g6-drop-{}", uuid::Uuid::new_v4()));
