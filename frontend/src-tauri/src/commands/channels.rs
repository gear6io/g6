...
pub async fn get_channels(state: State<'_, AppState>) -> Result<Vec<ChannelInfo>, String> {
    ...
    let hidden_dms: std::collections::HashSet<String> = {
        let events = query_relay(
            &state,
            &[serde_json::json!({
                "kinds": [g6_core_pkg::kind::KIND_DM_VISIBILITY],
                "#p": [&my_pubkey],
                "limit": 1,
            })],
        )
    ...
    #[cfg(debug_assertions)]
    {
        ...
        eprintln!(
            "g6-desktop: get_channels profile channels={} members={:?} member_meta={:?} open_meta={:?} member_counts={:?} last_message={:?} hidden_dm={:?} total={:?}",
            channels.len(),
            t_members,
