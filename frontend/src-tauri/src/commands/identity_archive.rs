...
/// `g6-sdk` lives on nostr 0.36; the desktop is on 0.37, so we bridge
...
pub(crate) fn extract_oa_owner(target_kind0: &nostr::Event) -> Option<(String, [String; 4])> {
    ...
    for tag in target_kind0.tags.iter() {
        ...
        match g6_sdk_pkg::nip_oa::verify_auth_tag(&json, &target_compat) {
            Ok(owner) => {
                let raw: [String; 4] = [
                    slice[0].clone(),
                    slice[1].clone(),
...
mod tests {
    ...
    fn kind0_with_auth(agent: &Keys, owner: &Keys) -> nostr::Event {
        // Compute auth tag via g6-sdk (nostr 0.36) and bridge.
        ...
        let tag_json =
            g6_sdk_pkg::nip_oa::compute_auth_tag(&owner_compat_keys, &agent_compat, "")
                .expect("compute_auth_tag");
        let compat_tag = g6_sdk_pkg::nip_oa::parse_auth_tag(&tag_json).unwrap();
