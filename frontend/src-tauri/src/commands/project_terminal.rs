...
fn merge_recovery_ref(expected_commit: &str) -> String {
    format!("refs/g6/merge-recovery/{expected_commit}")
}

fn merge_recovery_target_ref(target_commit: &str) -> String {
    format!("refs/g6/merge-recovery-target/{target_commit}")
}
...
mod tests {
    ...
    fn recovery_ref_is_namespaced_by_verified_commit() {
        ...
        assert_eq!(
            ...
            format!("refs/g6/merge-recovery/{commit}"),
        );
        assert_eq!(
            merge_recovery_target_ref(&commit),
            format!("refs/g6/merge-recovery-target/{commit}"),
        );
    }
}
