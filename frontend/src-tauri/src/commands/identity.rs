...
pub fn is_shared_identity() -> bool {
    std::env::var("GEAR6_SHARE_IDENTITY")
        .map(|v| v == "1")
        .unwrap_or(false)
        && std::env::var("GEAR6_PRIVATE_KEY")
            .ok()
...
pub fn decrypt_observer_event(
    ...
    g6_core_pkg::observer::decrypt_observer_payload(&keys, &event)
...
pub fn build_observer_control_event(
    ...
    let encrypted =
        g6_core_pkg::observer::encrypt_observer_payload(&keys, &agent_pubkey, &payload)
            .map_err(|error| format!("encrypt observer control failed: {error}"))?;
    let builder = g6_sdk_pkg::build_agent_observer_frame(
        &agent_pubkey_hex,
        &agent_pubkey_hex,
        g6_core_pkg::observer::OBSERVER_FRAME_CONTROL,
        &encrypted,
    )
...
pub async fn import_identity(
    ...
    tokio::task::spawn_blocking(move || {
        ...
        *state.keys.lock().map_err(|e| e.to_string())? = keys;
        ...
        eprintln!("g6-desktop: imported identity pubkey {}", pubkey_hex);
...
/// open.
///
/// Not available in shared-identity mode (`GEAR6_SHARE_IDENTITY=1`): the key
...
pub async fn sign_out(app: tauri::AppHandle) -> Result<(), String> {
    if is_shared_identity() {
        return Err(
            "Sign out isn't available while GEAR6_SHARE_IDENTITY provides your identity. Unset GEAR6_SHARE_IDENTITY and GEAR6_PRIVATE_KEY, then relaunch to sign out."
                .to_string(),
        );
    }
    ...
    if let Err(e) = crate::shutdown::shutdown_managed_agents(&app) {
        eprintln!("g6-desktop sign-out: agent shutdown: {e}");
    }
...
mod nostr_identity_binding_tests {
    ...
    fn build_nostr_identity_binding_event_signs_exact_shape() {
        ...
        assert!(tags.contains(&vec!["audience".into(), "g6:nostr-identity".into()]));
        assert!(tags.contains(&vec!["action".into(), "bind_nostr_identity".into(),]));
        assert!(tags.contains(&vec!["protocol".into(), "g6-nostr-identity".into(),]));
