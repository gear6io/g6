...
//! - `GEAR6_BUILD_OBSERVER_ARCHIVE_DEFAULT` was set at build time (internal
//!   builds bake in the flag via `build.rs`), **or**
//! - the running binary is using the dev nest (`~/.g6-dev`), which is the
...
//!
//! OSS prod builds (baked flag unset, prod nest `~/.g6`) return `false` —
...
/// binary is using the dev nest (`~/.g6-dev`). The frontend calls this
/// every startup to decide whether to reconcile the `owner_p` subscription.
#[tauri::command]
pub fn observer_archive_default_enabled() -> bool {
    option_env!("GEAR6_DESKTOP_BUILD_OBSERVER_ARCHIVE_DEFAULT").is_some()
...
mod tests {
    ...
    // is uninitialized → falls back to prod `~/.g6` (nest.rs:101-106), so
    // the compiled flag is the sole variable. No runner normalization needed.
    //
    // #[ignore]: requires GEAR6_TEST_EXPECTED_OBSERVER_ARCHIVE_DEFAULT to be
    ...
    fn test_observer_archive_default_enabled_matches_expected() {
        ...
        let expected_str = std::env::var("GEAR6_TEST_EXPECTED_OBSERVER_ARCHIVE_DEFAULT").expect(
            "GEAR6_TEST_EXPECTED_OBSERVER_ARCHIVE_DEFAULT must be set — \
        ...
        assert_eq!(
            ...
            "observer_archive_default_enabled() returned {result}, \
             expected {expected} (GEAR6_TEST_EXPECTED_OBSERVER_ARCHIVE_DEFAULT={expected_str:?})"
        );
    }
}
