...
use crate::managed_agents::{
    build_g6_agent_provider_defaults, default_agent_workdir, known_acp_runtime,
...
pub(super) async fn run_agent_models_command(
    ...
    let output = tokio::task::spawn_blocking(move || {
        ...
        cmd.arg("models")
            .arg("--json")
            .env("GEAR6_ACP_AGENT_COMMAND", &agent_command)
        ...
        // User env layering — written LAST so it overrides any Gear6-set env above.
        ...
        cmd.stdout(std::process::Stdio::piped())
            ...
            .output()
            .map_err(|e| format!("failed to spawn g6-acp models: {e}"))
    })
    .await
    ...
    if !output.status.success() {
        ...
        return Err(format!(
            "g6-acp models failed (exit {}): {stderr_redacted}",
