...
const GEAR6_RELEASE_IDENTIFIER_PREFIX: &str = "xyz.block.g6.app";
const SPROUT_RELEASE_IDENTIFIER: &str = "xyz.block.sprout.app";
const GEAR6_DEV_IDENTIFIER_PREFIX: &str = "xyz.block.g6.app.dev";
...
fn legacy_identifier(current_identifier: &str) -> Option<String> {
    if current_identifier.starts_with(GEAR6_DEV_IDENTIFIER_PREFIX) {
        Some(current_identifier.replacen(
            GEAR6_DEV_IDENTIFIER_PREFIX,
            SPROUT_DEV_IDENTIFIER_PREFIX,
            1,
        ))
    } else if current_identifier.starts_with(GEAR6_RELEASE_IDENTIFIER_PREFIX) {
        Some(current_identifier.replacen(
            GEAR6_RELEASE_IDENTIFIER_PREFIX,
...
/// data directory so the frontend can seed Gear6 localStorage before first
    ...
    tokio::task::spawn_blocking(move || {
        ...
        for database in databases {
            match read_legacy_workspace_storage_db(&database) {
                ...
                Err(error) => eprintln!(
                    "g6-desktop: legacy-local-storage-migration: {}: {error}",
                    database.display()
                ),
            }
        }
...
mod tests {
    ...
    fn legacy_identifier_maps_release_identifier() {
        assert_eq!(
            legacy_identifier("xyz.block.g6.app"),
    ...
    fn legacy_identifier_maps_dev_worktree_identifier() {
        assert_eq!(
            legacy_identifier("xyz.block.g6.app.dev.my-branch"),
