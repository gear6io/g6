...
pub(super) fn transcode_to_mp4(
    ...
    let output = std::env::temp_dir().join(format!("g6-transcode-{}.mp4", uuid::Uuid::new_v4()));
...
pub(super) fn transcode_heic_to_jpeg(
    ...
    // UUID-based temp path — unique across concurrent uploads.
    let output = std::env::temp_dir().join(format!("g6-heic-{}.jpg", uuid::Uuid::new_v4()));
...
pub(super) fn extract_poster_frame(
    ...
    let output = std::env::temp_dir().join(format!("g6-poster-{}.jpg", uuid::Uuid::new_v4()));
        ...
        || std::fs::metadata(&output).map_or(true, |m| m.len() == 0)
    {
        if !result.status.success() {
            let stderr = String::from_utf8_lossy(&result.stderr);
            eprintln!("g6-desktop: poster seek-to-1s failed, trying first frame: {stderr}");
        }
        let _ = std::fs::remove_file(&output);
        ...
        if !fallback.status.success() || !output.exists() {
            ...
            eprintln!("g6-desktop: poster frame extraction failed: {stderr}");
            let _ = std::fs::remove_file(&output);
...
pub(super) fn transcode_and_extract_poster(
    ...
    let poster_bytes = match extract_poster_frame(&transcoded, &ffmpeg_path) {
        ...
        Err(e) => {
            eprintln!("g6-desktop: poster extraction failed (non-fatal): {e}");
            None
        }
    };
...
mod tests {
    ...
    fn test_transcode_to_mp4_drops_source_metadata() {
        ...
        let source =
            std::env::temp_dir().join(format!("g6-metadata-test-{}.mp4", uuid::Uuid::new_v4()));
    ...
    fn test_transcode_heic_round_trip() {
        ...
        let heic_path =
            std::env::temp_dir().join(format!("g6-test-{}.heic", uuid::Uuid::new_v4()));
