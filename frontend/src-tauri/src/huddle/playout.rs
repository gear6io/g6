...
impl PeerSlot {
    fn new(peer_idx: u8, sink_mixer: &rodio::mixer::Mixer) -> Option<Self> {
        match PeerJitterBuffer::new(peer_idx) {
            ...
            Err(e) => {
                eprintln!("g6-desktop: jitter buffer init peer {peer_idx}: {e}");
                None
            }
        }
    }
                    ...
                    match slot.jitter.get_audio() {
                        Ok((samples, _vad)) => {
                            ...
                            if slot.player.len() >= PLAYOUT_QUEUE_HIGH_WATER {
                                eprintln!(
                                    "g6-desktop: playout queue high-water for peer {peer_idx} \
                        ...
                        Err(e) => {
                            eprintln!(
                                "g6-desktop: jitter get_audio peer {peer_idx}: {e}"
                            );
                        }
                    }
                }
                ...
                match msg {
                    Some(Ok(WsMsg::Binary(data))) => {
                        ...
                        let Some((header, opus_bytes)) = FrameHeader::parse(after_idx)
                            ...
                            eprintln!(
                                "g6-desktop: dropping malformed audio frame from peer {peer_idx} ({} bytes)",
                                data.len(),
                            );
                            continue;
                        };
                        ...
                        {
                            eprintln!(
                                "g6-desktop: jitter insert peer {peer_idx}: {err}"
                            );
                        } else {
