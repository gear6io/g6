...
as its own `g6 messages send` tool call: it is what breaks the silence.
Then send each following sentence the same way — one sentence per separate
`g6 messages send` call. Never hold a finished sentence back to bundle it
with the next one.
...
- Use your Gear6 tools proactively when asked."
    )
}
...
pub async fn add_agent_to_huddle(
    ...
    let (parent_added, parent_error) = {
        ...
        match submit_event(add_parent, state).await {
            ...
            Err(e) => {
                eprintln!(
                    "g6-desktop: add agent to parent channel failed (may already be member): {e}"
                );
                (false, Some(e))
            }
        }
