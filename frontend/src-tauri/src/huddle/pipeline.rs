...
pub(crate) async fn post_connect_setup(
    ...
    if let Err(e) = maybe_start_tts_pipeline(state).await {
        eprintln!("g6-desktop: TTS pipeline failed to start: {e}");
    }
        ...
        while let Some(t) = text_rx.recv().await {
            ...
            let builder =
                match events::build_message(channel_uuid, &t, None, &p_tags, &[], &[], &[]) {
                    Ok(b) => b,
                    Err(e) => {
                        eprintln!("g6-desktop: STT build_message: {e}");
                        continue;
                    }
                };
            ...
            let event = match builder.sign_with_keys(&keys) {
                Ok(e) => e,
                Err(e) => {
                    eprintln!("g6-desktop: STT sign event: {e}");
                    continue;
                }
            };
            ...
            let auth_header = match crate::relay::build_nip98_auth_header_for_keys(
                ...
                Ok(h) => h,
                Err(e) => {
                    eprintln!("g6-desktop: STT NIP-98 auth: {e}");
                    continue;
                }
            };
            ...
            match response {
                ...
                Ok(resp) => {
                    ...
                    eprintln!("g6-desktop: STT kind:9 post failed: {msg}");
                }
                Err(e) => {
                    eprintln!("g6-desktop: STT kind:9 post failed: {e}");
                }
            }
        }
    });
