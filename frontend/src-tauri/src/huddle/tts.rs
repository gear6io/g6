...
impl TtsPipeline {
    ...
    pub fn speak(&self, text: String) -> Result<(), String> {
        self.text_tx.try_send(text).map_err(|e| {
            eprintln!("g6-desktop: TTS queue saturated, dropping message: {e}");
...
fn tts_worker(
    ...
    let engine = match load_text_to_speech(&model_dir_str) {
        ...
        Err(e) => {
            eprintln!(
                "g6-desktop: TTS engine init failed (model_dir={}): {e}. TTS disabled.",
                model_dir.display()
            );
    ...
    let style = match load_voice_style(&voice_path) {
        ...
        Err(e) => {
            eprintln!(
                "g6-desktop: TTS voice style load failed ({voice_name}): {e}. TTS disabled."
            );
            drain_until_shutdown(text_rx, &shutdown);
            return;
        }
    ...
    // and discard the output so the first real utterance runs at warm-session speed.
    {
        ...
        match engine.synth_chunk("warmup", "en", &style, SYNTH_STEPS) {
            Ok(_) => eprintln!(
                "g6-desktop: TTS warmup completed in {:.0}ms",
                t.elapsed().as_millis()
            ),
            Err(e) => eprintln!(
                "g6-desktop: TTS warmup failed after {:.0}ms: {e} — first utterance may be slow",
                t.elapsed().as_millis()
            ),
        }
    }
    ...
    let sink_handle = match super::audio_output::open_output_sink_by_name(output_device.as_deref())
    {
        Ok(h) => h,
        Err(e) => {
            eprintln!("g6-desktop: TTS audio output failed: {e}. TTS disabled.");
            drain_until_shutdown(text_rx, &shutdown);
            return;
        }
    };
    ...
    let channels = match NonZero::new(1u16) {
        Some(c) => c,
        None => {
            eprintln!("g6-desktop: TTS channel count invariant violated");
            return;
        }
    };
    let rate = match NonZero::new(SAMPLE_RATE) {
        Some(r) => r,
        None => {
            eprintln!("g6-desktop: TTS sample rate invariant violated");
            return;
        }
    };
    ...
    if let Err(ref e) = monitor {
        ...
        eprintln!("g6-desktop: TTS barge-in monitor failed to spawn: {e}");
    }
    ...
    loop {
        ...
        for chunk in &chunks {
            ...
            match engine.synth_chunk(text, "en", &style, SYNTH_STEPS) {
                ...
                Ok(_) => {}
                Err(e) => {
                    eprintln!("g6-desktop: TTS synth failed: {e}");
                }
            }
        }
