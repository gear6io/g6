...
//!
//! Mental model:
//!   app launch → start_stt_download (background) → ~/.g6/models/parakeet-tdt-ctc-110m-en/
//!   app launch → start_tts_download (background) → ~/.g6/models/pocket-tts/
//!   STT pipeline → is_stt_ready() → stt_model_dir() → run inference
//!   TTS pipeline → is_tts_ready() → tts_model_dir() → run synthesis
//!
//! Models are downloaded once and cached. A version manifest (`.g6-model-manifest`)
...
//! `~/.g6/models/moonshine-tiny/` is removed best-effort once the new STT
...
const MANIFEST_FILENAME: &str = ".g6-model-manifest";
...
(https://github.com/k2-fsa/sherpa-onnx); Gear6 ships this conversion
unmodified.
...
Gear6 ships all ONNX/model artifacts and the reference voice WAV unmodified,
...
#[derive(Clone)]
struct ModelSlot {
    dir_name: &'static str,                  // subdir under ~/.g6/models/
        ...
        Fut: std::future::Future<Output = Result<(), String>> + Send,
    {
        ...
        tauri::async_runtime::spawn(async move {
            if let Err(e) = download_fn(http_client).await {
                eprintln!("g6-desktop: {name} download failed: {e}");
...
impl ModelManager {
    /// Create a new `ModelManager` rooted at `~/.g6/models/`.
    ///
    /// Returns `None` if the home directory cannot be resolved.
    pub fn new() -> Option<Self> {
        let models_dir = dirs::home_dir()?.join(".g6").join("models");
        Some(Self {
            models_dir,
    ...
    async fn download_stt_model(&self, http_client: reqwest::Client) -> Result<(), String> {
        ...
        eprintln!("g6-desktop: downloading STT model from {STT_DOWNLOAD_URL}");
        ...
        .await?;
        eprintln!("g6-desktop: downloaded {bytes} bytes, wrote to disk");
        ...
        eprintln!("g6-desktop: extracting STT archive…");
    ...
    ///
    /// Downloads files into `~/.g6/models/pocket-tts/`:
    ///   - five ONNX sessions (Pocket TTS + Mimi codec)
    ///   - `vocab.json` / `token_scores.json` for sherpa-onnx text conditioning
    ///   - upstream `LICENSE` plus Gear6's `MODEL_LICENSE.txt` attribution sidecar
    ...
    async fn download_tts_model(&self, http_client: reqwest::Client) -> Result<(), String> {
        ...
        for (i, (url, filename)) in downloads.iter().enumerate() {
            eprintln!("g6-desktop: downloading Pocket TTS {filename} from {url}");
            ...
            eprintln!("g6-desktop: downloaded {bytes} bytes ({filename}), wrote to disk");
        ...
        eprintln!(
            "g6-desktop: Pocket TTS model ready at {}",
...
///
/// Removes `~/.g6/models/moonshine-tiny/` if present (~70 MB on disk).
...
async fn cleanup_legacy_moonshine_dir(models_dir: &Path) {
    ...
    match tokio::fs::remove_dir_all(&legacy).await {
        Ok(()) => eprintln!(
            "g6-desktop: removed legacy STT model dir {}",
            legacy.display()
        ),
        Err(e) => eprintln!(
            "g6-desktop: could not remove legacy STT model dir {}: {e} \
