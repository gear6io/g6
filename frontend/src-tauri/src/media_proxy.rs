...
pub async fn spawn_media_proxy(http_client: reqwest::Client, app_handle: tauri::AppHandle) -> u16 {
    ...
    eprintln!("g6-desktop: media proxy listening on 127.0.0.1:{port}");
    port
}
...
/// This handler routes `g6-media://localhost/{path}` through reqwest, which
/// runs in the Tauri process and goes through WARP.
pub async fn handle_g6_media(
    app: &tauri::AppHandle,
