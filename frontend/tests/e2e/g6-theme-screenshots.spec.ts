...
const SHOTS = "test-results/g6-theme";
const THEME_STORAGE_KEY = "g6-theme";
...
async function expectGear6SidebarPalette(page: Page, mode: "light" | "dark") {
  const mutedColor =
  ...
  const sidebarScroller = page.locator(".g6-sidebar-scrollbar");
  ...
  await expect(page.locator("[data-g6-sidebar-secondary]").first()).toHaveCSS(
    "color",
    mutedColor,
  );
...
async function expectGear6ContentShadow(page: Page, mode: "light" | "dark") {
  const effects = await page.evaluate(() => {
    const shell = document.querySelector(".g6-huddle-shell");
    const content = document.querySelector("[data-g6-content-surface]");
    const shadowViewport = document.querySelector(
      "[data-g6-shadow-viewport]",
    );
    return {
...
async function expectGear6GradientPaint(
  page: Page,
  mode: "light" | "dark",
): Promise<string> {
  const paint = await page.evaluate(() => {
    const root = document.documentElement;
    const appSurface = document.querySelector(".g6-huddle-app-surface");
    const lightLayer = document.querySelector('[data-g6-gradient="light"]');
    const darkLayer = document.querySelector('[data-g6-gradient="dark"]');
    ...
    return {
      ...
      theme: root.getAttribute("data-g6-theme"),
  ...
  expect(paint.theme).toBe(mode === "light" ? "g6" : "g6-dark");
...
async function expectGear6SettingsPalette(page: Page, mode: "light" | "dark") {
  const mutedColor =
    mode === "light" ? "rgba(0, 0, 0, 0.4)" : "rgba(255, 255, 255, 0.4)";
  ...
  await expectGear6GradientPaint(page, mode);
...
async function expectAppliedGear6Theme(
  page: Page,
  themeName: "g6" | "g6-dark",
  storedTheme: "g6" | "g6-dark" = themeName,
) {
  const isDark = themeName === "g6-dark";
  await expect
    .poll(() =>
      page.evaluate((storageKey) => {
        ...
        return {
          ...
          isDark: root.classList.contains("dark"),
          g6Theme: root.getAttribute("data-g6-theme"),
          gradientTop: styles.getPropertyValue("--g6-gradient-top").trim(),
          gradientBottom: styles
            .getPropertyValue("--g6-gradient-bottom")
            .trim(),
        };
      }, THEME_STORAGE_KEY),
    )
...
test("g6 light sidebar gradient", async ({ page }) => {
  await seedTheme(page, "g6");
  await installMockBridge(page);
  await openChannel(page);
  await expectGear6GradientPaint(page, "light");
  await expectGear6SidebarPalette(page, "light");
  await expectGear6ContentShadow(page, "light");
  await expectIconlessSectionTitleAligned(page, "stream-list");
  await expectIconlessSectionTitleAligned(page, "dm-list");
  await waitForAnimations(page);
  await page
    .getByTestId("app-sidebar")
    .screenshot({ path: `${SHOTS}/01-g6-light-sidebar.png` });
});

test("g6 dark sidebar gradient", async ({ page }) => {
  await seedTheme(page, "g6-dark");
  await installMockBridge(page);
  await openChannel(page);
  await expectGear6GradientPaint(page, "dark");
  await expectGear6SidebarPalette(page, "dark");
  await expectGear6ContentShadow(page, "dark");
  await expectIconlessSectionTitleAligned(page, "stream-list");
  await expectIconlessSectionTitleAligned(page, "dm-list");
  await expect(page.locator("[data-g6-content-surface]")).toHaveCSS(
    "background-color",
    "rgb(26, 26, 26)",
  );
  await waitForAnimations(page);
  await page
    .getByTestId("app-sidebar")
    .screenshot({ path: `${SHOTS}/02-g6-dark-sidebar.png` });
});
...
test("appearance picker — system tab (Gear6 follows OS)", async ({ page }) => {
  await seedTheme(page, "g6");
  await installMockBridge(page);
...
test("appearance picker — light tab (Gear6)", async ({ page }) => {
  await seedTheme(page, "g6");
  await installMockBridge(page);
...
test("appearance picker — dark tab (Gear6 Dark)", async ({ page }) => {
  await seedTheme(page, "g6-dark");
  await installMockBridge(page);
...
test("settings nav uses Gear6 active pill + hover (light)", async ({ page }) => {
  await seedTheme(page, "g6");
  await installMockBridge(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.getByTestId("open-settings").click();
  ...
  // Appearance is the active section here; its nav row should carry the Gear6
  ...
  await expectGear6SettingsPalette(page, "light");
...
test("settings nav uses Gear6 active pill + hover (dark)", async ({ page }) => {
  await seedTheme(page, "g6-dark");
  await installMockBridge(page);
  await page.goto("/", { waitUntil: "domcontentloaded" });
  await page.getByTestId("open-settings").click();
  ...
  await expect(sidebar).toBeVisible({ timeout: 10_000 });
  await page.getByTestId("settings-nav-appearance").click();
  await expectGear6SettingsPalette(page, "dark");
...
test("appearance hides accent picker under Gear6", async ({ page }) => {
  await seedTheme(page, "g6");
  await installMockBridge(page);
  const panel = await openAppearance(page, "light");
  // The accent picker is hidden while a Gear6 theme is active. Its neutral
...
test("accent picker reveals/hides when toggling Gear6", async ({ page }) => {
  // Start on a non-Gear6 theme so the accent picker is present, then select the
  // Gear6 tile — the picker should animate out and unmount. Reselecting a
  // non-Gear6 tile brings it back. Asserts the presence toggle (the motion
  ...
  // Switch to Gear6 — picker should leave (allow the exit animation to settle).
  await page.getByTestId("theme-option-g6").click();
  await expect(page.getByTestId("accent-color-neutral")).toHaveCount(0);

  // Back to a non-Gear6 theme — picker returns.
  await page.getByTestId("theme-option-github-light").click();
  await expect(page.getByTestId("accent-color-neutral")).toBeVisible();
});

test("Gear6 light and dark modes apply live without a reload", async ({
  page,
}) => {
  await seedTheme(page, "g6");
  await installMockBridge(page);
  await openAppearance(page, "light");
  await expectAppliedGear6Theme(page, "g6");
  const lightGradient = await expectGear6GradientPaint(page, "light");

  await page.getByTestId("appearance-mode-dark").click();
  await expectAppliedGear6Theme(page, "g6-dark");
  const darkGradient = await expectGear6GradientPaint(page, "dark");
...
test("Gear6 follows native system theme changes without a reload", async ({
  page,
}) => {
  await seedTheme(page, "g6");
  await page.addInitScript(() => {
  ...
  await expectAppliedGear6Theme(page, "g6-dark", "g6");
  await expectGear6GradientPaint(page, "dark");

  await emitNativeThemeChange(page, "light");
  await expectAppliedGear6Theme(page, "g6", "g6");
  await expectGear6GradientPaint(page, "light");
});
