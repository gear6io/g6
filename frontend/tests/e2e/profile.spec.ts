    ...
    .poll(async () => {
      return page.evaluate((channelName) => {
        return (
          ...
          ).__GEAR6_E2E_HAS_MOCK_LIVE_SUBSCRIPTION__?.({ channelName }) ?? false
        );
      }, channelName);
    })
    .toBe(true);
...
test("keeps the saved profile description after a community round trip", async ({
  ...
  await page.addInitScript((seed) => {
    window.localStorage.setItem("g6-communities", JSON.stringify(seed));
    window.localStorage.setItem("g6-active-community-id", seed[0].id);
  }, communities);
  await page.goto("/");
...
test("nests the avatar edit button in a clipped notch", async ({ page }) => {
  // Under the Gear6 default theme the settings nav overrides `--sidebar-active`
  ...
  // only holds outside the Gear6 theme.
  await page.addInitScript(() => {
    window.localStorage.setItem("g6-theme", "github-light");
  });
  await page.goto("/");
...
test("uploads local profile avatar files before saving", async ({ page }) => {
  ...
  const pastedAvatarUrl = await page.evaluate(
    () => new URL("/g6.svg", window.location.href).href,
  );
...
test("renders emoji avatars with a static background layer", async ({
  ...
  await expect(avatarPreview).not.toHaveClass(/g6-avatar-squish/);
...
test("notification settings drive the Inbox badge and desktop alerts", async ({
  ...
  async function getAppBadgeCount() {
    return page.evaluate(() => {
      const win = window as Window & {
        __GEAR6_E2E_APP_BADGE_COUNT__?: number;
      };

      return win.__GEAR6_E2E_APP_BADGE_COUNT__ ?? 0;
    });
  }
  ...
  await page.evaluate(() => {
    ...
    win.__GEAR6_E2E_PUSH_MOCK_FEED_ITEM__?.({
      category: "mention",
  ...
  await expect
    .poll(() =>
      page.evaluate(() => {
        const win = window as Window & {
          __GEAR6_E2E_NOTIFICATIONS__?: Array<{
            body: string | null;
            title: string;
          }>;
        };

        return win.__GEAR6_E2E_NOTIFICATIONS__?.length ?? 0;
      }),
    )
    .toBe(1);
  ...
  const notifications = await page.evaluate(() => {
    ...
    return win.__GEAR6_E2E_NOTIFICATIONS__ ?? [];
  });
  ...
  const clickedNotification = await page.evaluate(() => {
    const win = window as Window & {
      __GEAR6_E2E_CLICK_NOTIFICATION__?: (index: number) => boolean;
    };

    return win.__GEAR6_E2E_CLICK_NOTIFICATION__?.(0) ?? false;
  });
...
test("opens settings with the keyboard shortcut and updates theme", async ({
  ...
  // Default is Gear6 in System mode; Playwright's default color scheme is
  // light, so the app boots with the light Gear6 theme.
  await expect
    .poll(() =>
  ...
  await expect
    .poll(() => page.evaluate(() => localStorage.getItem("g6-theme")))
    .toBe("github-light");
...
test("supports webview zoom keyboard shortcuts", async ({ page }) => {
  ...
  const getTextScaleState = () =>
    page.evaluate(() => ({
      ...
      storedScale: localStorage.getItem("g6:text-scale"),
      webviewZoom: (window as Window & { __GEAR6_E2E_WEBVIEW_ZOOM__?: number })
        .__GEAR6_E2E_WEBVIEW_ZOOM__,
    }));
