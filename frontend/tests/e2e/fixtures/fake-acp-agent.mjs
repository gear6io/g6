...
const wakeDelayMs = Number.parseInt(
  process.env.GEAR6_E2E_FAKE_ACP_WAKE_MS ?? "0",
  10,
);
if (!Number.isFinite(wakeDelayMs) || wakeDelayMs < 0) {
  throw new Error("GEAR6_E2E_FAKE_ACP_WAKE_MS must be a non-negative integer");
}
...
for await (const line of input) {
  ...
  switch (request.method) {
    ...
    case "session/prompt": {
      ...
      const cli = process.env.GEAR6_E2E_CLI_BIN;
      if (cli && channel) {
        const args = [
          "messages",
          "send",
