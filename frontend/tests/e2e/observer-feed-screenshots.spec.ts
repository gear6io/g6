...
async function waitForSeedHook(page: import("@playwright/test").Page) {
  await page.waitForFunction(
    () => typeof window.__GEAR6_E2E_SEED_OBSERVER_EVENTS__ === "function",
    null,
    { timeout: 10_000 },
  );
}
...
async function seedObserverEvents(
  ...
  await page.evaluate(
    ({ pubkey, evts }) => {
      window.__GEAR6_E2E_SEED_OBSERVER_EVENTS__?.({
        agentPubkey: pubkey,
        events: evts,
      });
    },
...
test.describe("observer feed screenshots", () => {
  ...
  test("11 — first-turn bundle: standalone system-prompt card + Checks ingress (per-turn context only)", async ({
    ...
    // (Gear6 event / Thread context) — no Base/System/Team Instructions/Core Memory/Channel Canvas sections.
    await seedObserverEvents(page, OBSERVER_AGENT_PUBKEY, [
      {
        seq: 1,
        timestamp: NOW,
    ...
    // Per-turn prompt context (Gear6 event / Thread context) does NOT appear
    ...
    // (Gear6 event, Thread context). Base/System/Team Instructions/Core Memory/Channel Canvas must NOT appear.
    ...
    // Only per-turn context sections (Gear6 event + Thread context) — no system-prompt sections.
    expect(sectionTitles.length).toBe(2);
    expect(sectionTitles[0]).toContain("Gear6 event");
