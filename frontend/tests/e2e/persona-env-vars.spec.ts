...
async function waitForInvokeBridge(page: import("@playwright/test").Page) {
  await page.waitForFunction(
    () => {
      ...
      return (
        typeof w.__GEAR6_E2E_INVOKE_MOCK_COMMAND__ === "function" ||
...
async function invokeTauri<T>(
  ...
  return page.evaluate(
    async ({ command: c, payload: p }) => {
      ...
      const invoke =
        w.__GEAR6_E2E_INVOKE_MOCK_COMMAND__ ?? w.__TAURI_INTERNALS__?.invoke;
...
test("env vars editor renders in PersonaDialog new-persona form", async ({
  ...
  // Initially empty (no rows — g6-agent with no provider has no required keys).
...
test("persona model options follow the selected LLM provider", async ({
  ...
  await expect(provider).toContainText("Gear6 Agent (default)");
