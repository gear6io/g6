...
/**
 * SCROLL-BACK latency profile for one channel (#g6-bugs) against a LIVE
 ...
 *   GEAR6_E2E_RELAY_URL=http://127.0.0.1:13000 \
 *   GEAR6_COMMUNITY_HOST=sprout-oss.stage.blox.sqprod.co \
 *   GEAR6_PERF_NSEC=nsec1... \
 *   npx playwright test --config=playwright.perf.config.ts scrollback-g6bugs.perf.ts
 */

const RELAY_HTTP =
  process.env.GEAR6_E2E_RELAY_URL ?? "https://sprout-oss.stage.blox.sqprod.co";
const NSEC = process.env.GEAR6_PERF_NSEC ?? "";
const COMMUNITY_HOST = process.env.GEAR6_COMMUNITY_HOST ?? "";
const TARGET_CHANNEL = process.env.GEAR6_PERF_CHANNEL ?? "g6-bugs";
const PAGES = Number(process.env.GEAR6_PERF_PAGES ?? 10);

const IDENTITY_OVERRIDE_KEY = "g6:e2e-identity-override.v1";
const ONBOARDING_PREFIX = "g6-onboarding-complete.v1:";
const WELCOME_PREFIX = "g6-welcome-channel-ensured.v2:";
...
function deriveIdentity(nsec: string) {
  ...
  if (decoded.type !== "nsec") throw new Error("GEAR6_PERF_NSEC is not an nsec");
...
test("MEASURE: scroll-back pagination latency in target channel", async ({
  ...
  if (!NSEC) throw new Error("Set GEAR6_PERF_NSEC to a real member nsec");
  ...
  await page.addInitScript(
    ({ ident, onboardingPrefix, welcomePrefix, relayUrl, overrideKey }) => {
      ...
      const w = window as unknown as { __GEAR6_E2E__?: Record<string, unknown> };
      w.__GEAR6_E2E__ = { ...(w.__GEAR6_E2E__ ?? {}), identity: ident };
    },
    {
      ident: identity,
