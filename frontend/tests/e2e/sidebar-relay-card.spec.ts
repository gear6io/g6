...
async function setChannelsReadError(page: Page, error: string | null) {
  await page.evaluate((nextError) => {
    const testWindow = window as Window & {
      __GEAR6_E2E__?: { mock?: { channelsReadError?: string } };
    };
    ...
    if (nextError === null) {
      delete testWindow.__GEAR6_E2E__.mock.channelsReadError;
      return;
    }

    testWindow.__GEAR6_E2E__.mock.channelsReadError = nextError;
  }, error);
}
  ...
  if (state !== "connected") {
    ...
    await page.waitForFunction(() => {
      const win = window as Window & {
        __GEAR6_E2E_GET_RELAY_CONNECTION_STATE__?: () => string;
        __GEAR6_E2E_SET_RELAY_CONNECTION_STATE__?: unknown;
      };
      return (
        typeof win.__GEAR6_E2E_SET_RELAY_CONNECTION_STATE__ === "function" &&
        typeof win.__GEAR6_E2E_GET_RELAY_CONNECTION_STATE__ === "function" &&
        win.__GEAR6_E2E_GET_RELAY_CONNECTION_STATE__() === "connected"
      );
    });
  } else {
