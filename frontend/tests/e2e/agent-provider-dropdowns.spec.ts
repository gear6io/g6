 ...
 *       empty baked env, simulating an OSS install with no GEAR6_AGENT_PROVIDER).
 *
 ...
 *       runtime (g6-agent) and model discovery runs → model combobox is non-empty.
...
test.describe("agent provider dropdown screenshots", () => {
  ...
  // GEAR6_AGENT_PROVIDER is baked and hideProviderIds is empty → v1 appears.
  ...
  // default runtime (g6-agent via getDefaultPersonaRuntime) and model
  ...
  test("03-builtin-edit-runtime-seeded", async ({ page }) => {
    ...
    // must have run and selected the app default (g6-agent in the mock catalog).
