...
async function waitForBridge(page: import("@playwright/test").Page) {
  await page.waitForFunction(
    () =>
      typeof (window as Window & { __GEAR6_E2E_SEED_ACTIVE_TURNS__?: unknown })
        .__GEAR6_E2E_SEED_ACTIVE_TURNS__ === "function",
    null,
    { timeout: 10_000 },
  );
}
...
async function seedTurns(
  ...
  await page.evaluate((seeds) => {
    ...
    for (const seed of seeds) win.__GEAR6_E2E_SEED_ACTIVE_TURNS__?.(seed);
  }, turns);
}
