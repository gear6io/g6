...
async function openNostrBind(
  ...
  await page.waitForFunction(
    () =>
      ...
      ).__GEAR6_E2E_INVOKE_MOCK_COMMAND__ === "function",
  );
  ...
  await page.addInitScript(
    ({ fail }) => {
      const testWindow = window as Window & {
        __GEAR6_E2E_CLIPBOARD_TEXT__?: string;
      };
      Object.defineProperty(navigator, "clipboard", {
        ...
        value: {
          writeText: async (text: string) => {
            ...
            testWindow.__GEAR6_E2E_CLIPBOARD_TEXT__ = text;
          },
        },
      });
    },
...
async function installShakeCounter(page: Page) {
  await page.addInitScript(() => {
    const testWindow = window as Window & {
      __GEAR6_E2E_CODE_SHAKE_CALLS__?: number;
    };
    testWindow.__GEAR6_E2E_CODE_SHAKE_CALLS__ = 0;
    ...
    Element.prototype.animate = function animate(keyframes, options) {
      if (
        ...
        testWindow.__GEAR6_E2E_CODE_SHAKE_CALLS__ =
          (testWindow.__GEAR6_E2E_CODE_SHAKE_CALLS__ ?? 0) + 1;
      }
...
async function shakeCount(page: Page): Promise<number> {
  return page.evaluate(
    () =>
      ...
      ).__GEAR6_E2E_CODE_SHAKE_CALLS__ ?? 0,
  );
}
...
test("returns a signed response in the callback fragment after consent", async ({
  ...
  await openNostrBind(page, {
    ...VALID_REQUEST,
    callbackUrl: "https://admin.example.com/g6?source=bind#stale",
      ...
      page.evaluate(
        () =>
          (
            (
              window as Window & {
                __GEAR6_E2E_COMMAND_LOG__?: Array<{ command: string }>;
              }
            ).__GEAR6_E2E_COMMAND_LOG__ ?? []
  ...
  expect(callbackUrl.pathname).toBe("/g6");
  expect(callbackUrl.search).toBe("?source=bind");
  expect(callbackUrl.searchParams.has("g6_bind")).toBe(false);
  expect(callbackUrl.hash).toMatch(/^#g6_bind=v1\.[A-Za-z0-9_-]+$/);
});
...
test("opens the manual fallback when returning to the browser fails", async ({
  ...
  await openNostrBind(
    ...
    {
      ...VALID_REQUEST,
      callbackUrl: "https://admin.example.com/g6",
      returnMode: "browser_fragment_v1",
    },
...
test("keeps the signed response available when clipboard access fails", async ({
  ...
  await expect(
    page
      ...
      .getByText("Gear6 couldn't access the clipboard. Try again."),
  ).toBeVisible();
