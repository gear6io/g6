...
// The fresh-profile default is the Gear6 theme, which pins the neutral accent
// regardless of the stored accent color. Accent-driven review foreground
// assertions must run on a non-Gear6 theme for the seeded accent to apply.
...
async function installVideoReviewHarness(
  ...
  await page.addInitScript(
    ({ accentColor, themeName }) => {
      if (themeName) {
        window.localStorage.setItem("g6-theme", themeName);
      }
      window.localStorage.setItem("g6-accent-color", accentColor);
