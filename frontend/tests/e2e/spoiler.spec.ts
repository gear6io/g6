...
const IMAGE_URL = "http://127.0.0.1:4173/g6.svg";
...
type MockFeedWindow = Window &
  typeof globalThis & {
    __GEAR6_E2E_PUSH_MOCK_FEED_ITEM__?: (item: MockFeedItem) => MockFeedItem;
  };
      ...
      input.evaluate(() =>
        Array.from(
          document.querySelectorAll(
            '[data-testid="message-input"] .g6-spoiler[data-spoiler]',
          ),
...
test("image attachments can be marked and sent as hidden spoilers", async ({
  ...
  const spoilerBlock = lastMessage.locator(".g6-spoiler--block");
...
test("text spoiler stays usable while attachment upload is pending", async ({
  ...
  await expect(input.locator(".g6-spoiler[data-spoiler]")).toContainText(
    "pending secret",
  );
...
test("hidden spoiler links reveal without opening on the first click", async ({
  ...
  const lastMessage = page.getByTestId("message-row").last();
  const spoiler = lastMessage.locator(".g6-spoiler").first();
...
test("hidden spoilers stay masked on hover and focus until reveal", async ({
  ...
  const lastMessage = page.getByTestId("message-row").last();
  const spoiler = lastMessage.locator(".g6-spoiler").first();
  const content = spoiler.locator(".g6-spoiler__content");
  const particles = spoiler.locator(".g6-spoiler__particles");
...
test("non-interactive inbox preview spoilers let row clicks pass through", async ({
  ...
  const spoiler = item.locator(".g6-spoiler").first();
