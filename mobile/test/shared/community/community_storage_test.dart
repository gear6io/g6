...
import 'package:g6/shared/community/community.dart';
import 'package:g6/shared/community/community_storage.dart';
  ...
  group('CommunityStorage', () {
    ...
    group('migration', () {
      test('migrates saved workspaces to communities', () async {
        ...
        fakeSecure['g6_workspaces'] = jsonEncode([legacy.toJson()]);
        fakeSecure['g6_active_workspace_id'] = legacy.id;

        final loaded = await storage.loadAll();

        expect(loaded.single.id, legacy.id);
        expect(await storage.loadActiveId(), legacy.id);
        expect(fakeSecure['g6_communities'], isNotNull);
        expect(fakeSecure['g6_workspaces'], isNull);
        expect(fakeSecure['g6_active_workspace_id'], isNull);
      });

      test('migrates legacy keys to community on first load', () async {
        fakeSecure['g6_relay_url'] = 'https://legacy.example.com';
        fakeSecure['g6_token'] = 'legacy_token';
        fakeSecure['g6_pubkey'] = 'legacy_pub';
        fakeSecure['g6_nsec'] = 'legacy_nsec';
        ...
        expect(fakeSecure['g6_relay_url'], isNull);
        expect(fakeSecure['g6_token'], isNull);
        expect(fakeSecure['g6_pubkey'], isNull);
        expect(fakeSecure['g6_nsec'], isNull);
      ...
      test('migration generates name from localhost URL', () async {
        fakeSecure['g6_relay_url'] = 'http://localhost:3000';
        fakeSecure['g6_token'] = 'tok';
