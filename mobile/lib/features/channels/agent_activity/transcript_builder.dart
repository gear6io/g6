...
final _g6ToolNames = <String>{..._g6ReadTools, ..._g6WriteTools};

final _g6ToolNamesByLength = _g6ToolNames.toList()
  ..sort((a, b) => b.length.compareTo(a.length));

final _g6ToolTitleAliases = <(RegExp, String)>[
...
String? _findGear6ToolName(String value, bool includeShortNames) {
  final alias = _findGear6ToolAlias(value);
  if (alias != null) return alias;

  final normalized = _normalizeToolNameText(value);
  for (final name in _g6ToolNamesByLength) {
...
String? _findGear6ToolAlias(String value) {
  final normalizedPhrase = value
      .trim()
      .toLowerCase()
      .replaceAll(RegExp(r'[_-]+'), ' ')
      .replaceAll(RegExp(r'\s+'), ' ');
  for (final (pattern, name) in _g6ToolTitleAliases) {
...
_parsePromptText(String text) {
  ...
  for (final section in sections) {
    if (section.title.toLowerCase().startsWith('gear6 event')) {
...
({String title, String toolName, String? g6ToolName}) _extractToolIdentity(
  Map<String, dynamic> update,
) {
  final candidates = _collectToolNameCandidates(update);
  String? knownName;
  for (final c in candidates) {
    knownName = _findGear6ToolName(c, true);
    if (knownName != null) break;
  }
  knownName ??= _findGear6ToolName(_safeJsonEncode(update), false);
...
List<TranscriptItem> buildTranscript(List<ObserverFrame> events) {
  ...
  void upsertTool(
    ...
    final existing = itemsById[id];
    final canonicalGear6ToolName =
        g6ToolName ?? _findGear6ToolName(toolName, true);
    if (existing is ToolItem) {
      if (!_isGenericToolTitle(title)) {
        existing.title = title;
      }
      if (canonicalGear6ToolName != null) {
        existing.g6ToolName = canonicalGear6ToolName;
        existing.toolName = canonicalGear6ToolName;
      } else if (existing.g6ToolName == null &&
    ...
    final item = ToolItem(
      id: id,
      title: title,
      toolName: canonicalGear6ToolName ?? toolName,
      g6ToolName: canonicalGear6ToolName,
      status: status,
      args: args,
      result: result,
      isError: isError,
